...
- **Unified email backend selection** *(experimental)*: `hop3 server email backend <relay|catch|direct>` names the backend every declaring app inherits. `relay` (a provider or corporate smarthost) is the shipped backend — also spelled `server email set`; `catch` (a dev sink) and `direct` (a self-hosted MTA) are planned and fail loud until built, rather than pretending to be configured.
