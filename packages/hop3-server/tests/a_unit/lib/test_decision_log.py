...
class TestLogConfigDecision:
    """Tests for DecisionLogger.log_config_decision()."""

    def test_overrode_sets_override_category_and_details(self, captured_logs):
        """An overrode source yields OVERRIDE plus an overrode detail."""
        logger = DecisionLogger()

        logger.log_config_decision(
            "web", "hop3.toml", "overrides Procfile", overrode="Procfile"
        )

        decision = logger.decisions[0]
        assert decision.type is DecisionType.CONFIG_SOURCE
        assert decision.chosen == "web from hop3.toml"
        assert decision.reason_category is DecisionReason.OVERRIDE
        assert decision.details == {"overrode": "Procfile"}

    def test_no_override_uses_default_category(self, captured_logs):
        """Without overrode the category is DEFAULT and details is None."""
        logger = DecisionLogger()

        logger.log_config_decision("worker", "Procfile", "not in hop3.toml")

        decision = logger.decisions[0]
        assert decision.chosen == "worker from Procfile"
        assert decision.reason_category is DecisionReason.DEFAULT
        assert decision.details is None


class TestLogWorkerDecision:
    """Tests for DecisionLogger.log_worker_decision()."""

    def test_overrode_sets_override_category_and_details(self, captured_logs):
        """An overrode source yields OVERRIDE plus an overrode detail."""
        logger = DecisionLogger()

        logger.log_worker_decision(
            "web", "hop3.toml", "overrides Procfile", overrode="Procfile"
        )

        decision = logger.decisions[0]
        assert decision.type is DecisionType.WORKER_SOURCE
        assert decision.chosen == "'web' from hop3.toml"
        assert decision.reason_category is DecisionReason.OVERRIDE
        assert decision.details == {"overrode": "Procfile"}
    ...
    def test_no_override_uses_default_category(self, captured_logs):
        ...
        logger.log_worker_decision("worker", "Procfile", "not in hop3.toml")
...
class TestGetSummary:
    """Tests for DecisionLogger.get_summary()."""

    def test_summary_when_empty(self):
        """An empty logger reports that no decisions were logged."""
        logger = DecisionLogger()

        assert logger.get_summary() == "No decisions logged."

    def test_summary_lists_all_decisions(self, captured_logs):
        """The summary has a header line plus one indented line per decision."""
        logger = DecisionLogger()
        ...
        assert summary == (
            ...
            "  [builder] Using 'local': r1\n"
            "  [toolchain] Using 'python': r2"
        )
