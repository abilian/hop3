...
class TestNginxProxyPlugin:
    ...
    def test_nginx_proxy_deployment(
        self, proxy_container: dict, test_app_dir: Path, request
    ):
        """Test deploying an app with Nginx proxy."""
        verify_proxy_deployment(proxy_container, test_app_dir, request)
...
def verify_proxy_deployment(
    container_info: dict, test_app_dir: Path, request: Any = None
) -> None:
    ...
    Args:
        ...
        request: pytest request; used to expose the deployed app to the
            bundle-on-failure finalizer (ADR 043).
    """
    proxy_type = container_info["proxy_type"]
    app_name = f"proxy-test-{proxy_type}-{int(time.time())}"

    # Configure the virtual host. nginx renders `server_name $HOST_NAME` from
    # the app's HOST_NAME env var (hop3/plugins/proxy/nginx/_setup.py), so the
    # Host-header probe below only matches when HOST_NAME is set. The old
    # NGINX_SERVER_NAME var was never read by the vhost — that is why this test
    # used to 502 forever (and was disabled).
    hostname = f"{app_name}.test.local"
    if request is not None:
        request.node.hop3_app = app_name
        request.node.hop3_container = container_info["container"]
