...
> **Status (ADR 043 Phase 1):** `test_proxy_plugins.py` is still disabled (`xxx_` prefix → not collected).
> Two blockers were found:
> 1. **Fixed** — the test set `NGINX_SERVER_NAME`, but the nginx vhost renders `server_name $HOST_NAME`, so the Host-header probe never matched. Now sets `HOST_NAME`.
> 2. **Open** — the deploy step fails with `Secsh channel open FAILED: Administratively prohibited`: the e2e container's sshd refuses TCP forwarding, so the SSH-tunnel deploy can't reach `localhost:8000`. Fix the container `sshd_config` (allow `AllowTcpForwarding`) before renaming `xxx_ → test_`.
>
> The bundle-on-failure finalizer (`conftest.py::bundle_on_failure`) is wired and verified: on the deploy failure it collected a diagnostic bundle and `hop3-test why <run-id>` replays it.
