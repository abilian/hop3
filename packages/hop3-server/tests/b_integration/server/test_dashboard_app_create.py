...
def test_client(tmp_path: Path, monkeypatch):
    ...
    """
    # CRITICAL: Set environment variable BEFORE create_app() so
    # get_session_factory() picks up this database.
    # Use a per-test file DB under tmp_path (NOT a shared-cache in-memory DB):
    # a fixed in-memory name leaks rows across tests in the same process — it
    # previously polluted test_dashboard_views' "empty dashboard" assertion.
    # A unique file per test is shared across the TestClient threadpool and
    # fully isolated.
    db_path = tmp_path / "test.db"
    monkeypatch.setenv("HOP3_DATABASE_URI", f"sqlite:///{db_path}")
