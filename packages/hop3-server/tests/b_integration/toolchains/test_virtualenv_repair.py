...
def _stub_pip_install(toolchain, monkeypatch) -> None:
    ...
    no network) while the ``pip install --upgrade pip setuptools`` step is a
    no-op — keeping the test hermetic.
    """
    ...
    def shell_no_pip(cmd, *args, **kwargs):
        if "pip install" in cmd:
            return None
        return orig_shell(cmd, *args, **kwargs)
...
def test_broken_virtualenv_is_recreated(tmp_path: Path, monkeypatch):
    ...
    _stub_pip_install(toolchain, monkeypatch)
