...
def _write_secret_config(config_file: Path, config_data: dict) -> None:
    ...
    M1). ``mkstemp`` creates the temp file 0600-from-birth; ``os.replace`` swaps
    it in atomically (also tightening a pre-existing loose-mode config file).
    Raises on any failure so the caller can abort loud.
    """
    config_file.parent.mkdir(parents=True, exist_ok=True)
    fd, tmp = tempfile.mkstemp(
        prefix=".hop3-server.toml.", suffix=".tmp", dir=config_file.parent
    )
    try:
...
class SetupCmd(Command):
    ...
    def setup_secret_key(self, *, verbose: bool = False) -> None:
        ...
        # Save to config file. This file holds the JWT signing key, so it must
        # never exist world-readable — not even in the create→chmod window — and
        # a failed write must not leave an exposed or half-written secret
        # behind (audit M1, CWE-732). Fail loud instead of the old
        # warn-and-continue that echoed the key and asked for a manual chmod.
        try:
            config_data = toml.load(config_file) if config_file.exists() else {}
            config_data["HOP3_SECRET_KEY"] = secret_key
            _write_secret_config(config_file, config_data)
        except Exception as e:
            msg = f"Could not securely write HOP3_SECRET_KEY to {config_file}: {e}"
