...
class DockerTarget(DeploymentTarget):
    ...
    def _deploy_and_start(self) -> TargetInfo:
        ...
        try:  # ruff:ignore[too-many-statements-in-try-clause] — the diagnostic-save-on-any-failure pattern: the except clause's job is to call `_save_diagnostics_on_error()` no matter which startup step blew up (deploy, service start, health check). Narrowing the try would force the same diagnostics call to be sprinkled at every failure site.
