...
"""Demo 41: Piwigo Photo Gallery.

Demonstrates deploying Piwigo, a self-hosted photo gallery,
...
# Demo metadata
TITLE = "Demo 41: Piwigo Photo Gallery"
DESCRIPTION = """
Demonstrates deploying Piwigo with Hop3:
  - Docker-based deployment (linuxserver image)
...
def run(ctx: DemoContext) -> None:
    ...
    # Show app structure
    print_header("Deploying Piwigo Photo Gallery")

    show_app_structure(
        APP_NAME,
        [
            ("Dockerfile", "Docker image based on linuxserver/piwigo"),
            ("hop3.toml", "Hop3 configuration with [[provider]] section"),
        ],
    )
    print_info("Piwigo is a photo gallery software for the web.")
    ...
    wait_for_app(seconds=15, message="Waiting for Piwigo to start...")
    check_app_status(ctx, APP_NAME)

    # Test main endpoint
    print_header("Step 5: Test Application")
    # Piwigo installation page contains "Piwigo"
    test_app_via_curl(ctx, app_url, expected_content="Piwigo")
    print_success("Piwigo is running!")
    print_blank()
    print_info("Visit the URL to complete the installation wizard.")
    print_info("Use the MySQL credentials from DATABASE_URL for database setup.")
    pause(ctx.pause_between_steps)
    ...
    cleanup_app(ctx, APP_NAME, app_url)
    print_success("Demo 41 completed: Piwigo deployed successfully.")
