...
Demonstrates deploying WordPress, the world's most popular CMS,
...
# Demo metadata
TITLE = "Demo 37: WordPress"
DESCRIPTION = """
Demonstrates deploying WordPress with Hop3:
...
def run(ctx: DemoContext) -> None:
    ...
    # Show app structure
    print_header("Deploying WordPress")

    show_app_structure(
        APP_NAME,
        [
            ("Dockerfile", "Docker image based on wordpress:6.7-apache"),
            ("start.sh", "Startup script with MySQL configuration"),
            ("hop3.toml", "Hop3 configuration with [[provider]] section"),
        ],
    )
    print_info("WordPress is the world's most popular content management system.")
    print_info("Powers over 40% of all websites on the internet.")
    print_blank()
    pause(ctx.pause_between_steps)
    ...
    wait_for_app(seconds=30, message="Waiting for WordPress to start...")
    check_app_status(ctx, APP_NAME)

    # Test main endpoint
    print_header("Step 5: Test Application")
    # WordPress shows installation page on first visit
    test_app_via_curl(ctx, app_url, expected_content="WordPress")
    print_success("WordPress is running!")
    print_blank()
    print_info("Visit the URL to complete WordPress installation.")
    pause(ctx.pause_between_steps)
    ...
    cleanup_app(ctx, APP_NAME, app_url)
    print_success("Demo 37 completed: WordPress deployed successfully.")
