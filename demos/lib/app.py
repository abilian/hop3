...
def _get_app_config(app_name: str) -> dict[str, str]:
    ...
    if result.returncode == 0 and result.stdout:
        ...
        for line in lines:
            ...
            if stripped.startswith("Key") or stripped.startswith("-"):
                data_started = True
                continue
            if not data_started:
                continue
...
def wait_for_app(seconds: int = 5, message: str = "Waiting for application to start...") -> None:
...
def test_app_via_curl(
    ...
    for attempt in range(max_retries):
        ...
        # Check if we got a 502 Bad Gateway (app not ready yet)
        if "502 Bad Gateway" in result.stdout or "Connection refused" in str(result.stderr):
            if attempt < max_retries - 1:
                print_info(f"  App not ready yet, retrying in {retry_delay}s... ({attempt + 1}/{max_retries})")
