...
"""Demo 43: EasyAppointments.

Demonstrates deploying EasyAppointments, an open-source appointment scheduling tool,
...
# Demo metadata
TITLE = "Demo 43: EasyAppointments"
DESCRIPTION = """
Demonstrates deploying EasyAppointments with Hop3:
...
DEFAULT_HOSTNAME = "demo43.hop"
MYSQL_NAME = "demo43-db"
...
def run(ctx: DemoContext) -> None:
    ...
    show_app_structure(
        APP_NAME,
        [
            ("Dockerfile", "Docker image based on alextselegidis/easyappointments"),
            ("start.sh", "Startup script with MySQL configuration"),
            ("hop3.toml", "Hop3 configuration with [[provider]] section"),
        ],
    )
    print_info("EasyAppointments is an open-source appointment scheduling application.")
    ...
    wait_for_app(seconds=15, message="Waiting for EasyAppointments to start...")
    check_app_status(ctx, APP_NAME)

    # Test main endpoint
    print_header("Step 5: Test Application")
    # EasyAppointments installation page contains "Easy!Appointments"
    test_app_via_curl(ctx, app_url, expected_content="Easy!Appointments")
    print_success("EasyAppointments is running!")
    print_blank()
    print_info("Visit the URL to complete the installation wizard.")
    pause(ctx.pause_between_steps)
    ...
    cleanup_app(ctx, APP_NAME, app_url)
    print_success("Demo 43 completed: EasyAppointments deployed successfully.")
