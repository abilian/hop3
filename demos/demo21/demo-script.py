...
Demonstrates deploying HedgeDoc, a collaborative markdown editor,
...
# Demo metadata
TITLE = "Demo 21: HedgeDoc"
DESCRIPTION = """
Demonstrates deploying HedgeDoc with Hop3:
...
DEFAULT_HOSTNAME = "demo21.hop"
POSTGRES_NAME = "demo21-db"
...
def run(ctx: DemoContext) -> None:
    ...
    # Show app structure
    print_header("Deploying HedgeDoc")

    show_app_structure(
        APP_NAME,
        [
            ("Dockerfile", "Docker image with HedgeDoc"),
            ("start.sh", "Startup script"),
            ("hop3.toml", "Hop3 configuration"),
        ],
    )
    print_info("HedgeDoc is a collaborative markdown editor (formerly CodiMD).")
    print_blank()
    pause(ctx.pause_between_steps)
    ...
    wait_for_app(seconds=15, message="Waiting for HedgeDoc to start...")
    check_app_status(ctx, APP_NAME)

    # Test main endpoint
    print_header("Step 5: Test Application")
    test_app_via_curl(ctx, app_url, expected_content="HedgeDoc")
    print_success("HedgeDoc is running!")
    pause(ctx.pause_between_steps)

    # Test health endpoint (HedgeDoc status returns JSON with stats)
    print_header("Step 6: Health Check")
    test_app_via_curl(ctx, f"{app_url}/status", expected_content="onlineNotes")
    ...
    cleanup_app(ctx, APP_NAME, app_url)
    print_success("Demo 21 completed: HedgeDoc deployed successfully.")
