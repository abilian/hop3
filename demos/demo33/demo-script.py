...
section. Hop3 reads that declaration at deploy time, provisions the PostgreSQL
...
Demonstrates declarative addon provisioning via hop3.toml:
  - The app declares its PostgreSQL dependency with an [[addons]] section
...
def run(ctx: DemoContext) -> None:
    ...
    print_info("The app declares a PostgreSQL dependency in hop3.toml via [[addons]].")
    ...
    print_blank()
    print_info('Note the [[addons]] section: type = "postgres".')
    pause(ctx.pause_between_steps)

    # Deploy — the declared addon is auto-provisioned and DATABASE_URL is wired in.
    print_header("Step 1: Deploy (PostgreSQL auto-provisioned)")
    ...
    # The [[addons]] declaration provisioned PostgreSQL at deploy, so the app is
    ...
    print_success("App auto-connected to PostgreSQL via [[addons]].")
    pause(ctx.pause_between_steps)
    ...
    print_blank()
    print_success("Demo 33 completed: declarative PostgreSQL auto-provisioning.")
