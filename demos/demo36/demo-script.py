...
Demonstrates deploying Kanboard, a self-hosted Kanban project management tool,
...
# Demo metadata
TITLE = "Demo 36: Kanboard"
DESCRIPTION = """
Demonstrates deploying Kanboard with Hop3:
  - Docker-based deployment
  - MySQL addon for data storage
  - Self-hosted Kanban boards
  - Simple project management
"""

APP_NAME = "demo36"
APP_DIR = Path(__file__).parent / "app"
DEFAULT_HOSTNAME = "demo36.hop"
MYSQL_NAME = "demo36-db"
...
def run(ctx: DemoContext) -> None:
    ...
    # Show app structure
    print_header("Deploying Kanboard")

    show_app_structure(
        APP_NAME,
        [
            ("Dockerfile", "Docker image based on kanboard/kanboard"),
            ("start.sh", "Startup script with MySQL configuration"),
            ("hop3.toml", "Hop3 configuration with [[provider]] section"),
        ],
    )
    print_info("Kanboard is a simple, visual Kanban project management tool.")
    ...
    wait_for_app(seconds=20, message="Waiting for Kanboard to start...")
    check_app_status(ctx, APP_NAME)

    # Test main endpoint
    print_header("Step 5: Test Application")
    # Kanboard login page contains "Sign in" button text
    test_app_via_curl(ctx, app_url, expected_content="Sign in")
    print_success("Kanboard is running!")
    print_blank()
    ...
    cleanup_app(ctx, APP_NAME, app_url)
    print_success("Demo 36 completed: Kanboard deployed successfully.")
