...
"""Demo 16: Docker + Redis Addon.

Demonstrates deploying a Docker-based application with Redis addon.
"""
...
# Demo metadata
TITLE = "Demo 16: Docker + Redis"
DESCRIPTION = """
Demonstrates Docker deployment with Redis addon:
  - Building and deploying a Docker container
  - Creating a Redis instance via addon
  - Connecting the containerized app to Redis
  - Testing counter and caching operations
"""

APP_NAME = "demo16"
APP_DIR = Path(__file__).parent / "app"
DEFAULT_HOSTNAME = "demo16.hop"
...
def run(ctx: DemoContext) -> None:
    ...
    from lib.server import ensure_docker, ensure_redis

    app_hostname = DEFAULT_HOSTNAME
    app_url = f"https://{app_hostname}"

    # Ensure Docker and Redis are available
    ensure_docker(ctx)
    ensure_redis(ctx)
    ...
    # Show app structure
    print_header("Deploying Docker + Redis Application")
    ...
    print_info("This demo shows a Docker container connecting to a Redis addon.")
    print_blank()
    pause(ctx.pause_between_steps)

    # Show app.py
    show_file_content(APP_DIR / "app.py", "Application code (app.py):", max_lines=50)
    pause(ctx.pause_between_steps)
    ...
    test_app_via_curl(ctx, app_url, expected_content="Welcome to demo16")
    pause(ctx.pause_between_steps)
    ...
    if redis_available:
        ...
        print_step("Redeploying container with REDIS_URL...")
        redeploy_app(ctx, APP_NAME, APP_DIR)
        ...
        print_success("Docker container connected to Redis!")
        pause(ctx.pause_between_steps)
        ...
        test_app_via_curl(
            ctx, f"{app_url}/cache/set/greeting/hello-docker", expected_content='"action":"set"'
        )

        print_step("Getting cache value...")
        test_app_via_curl(
            ctx, f"{app_url}/cache/get/greeting", expected_content="hello-docker"
        )
    ...
    if redis_available:
        print_success("Demo 16 completed: Docker + Redis workflow demonstrated.")
    else:
        print_success("Demo 16 completed: Docker app deployed (Redis unavailable).")
