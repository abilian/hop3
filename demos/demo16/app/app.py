...
"""Demo 16: Docker app with Redis addon.

A Flask application running in Docker that connects to Redis
...
def home():
    ...
    return jsonify({
        "app": "demo16",
        "type": "docker",
        "message": "Welcome to demo16 - Docker + Redis!",
...
def get_counter():
    ...
    try:
        client = get_redis_client()
        value = client.get("demo16:counter")
        return jsonify({
...
def increment_counter():
    ...
    try:
        client = get_redis_client()
        new_value = client.incr("demo16:counter")
        return jsonify({
            "counter": new_value,
            "action": "incremented",
        })
...
def cache_set(key, value):
    ...
    try:
        client = get_redis_client()
        client.setex(f"demo16:cache:{key}", 60, value)
        return jsonify({
            "key": key,
            "value": value,
            "ttl": 60,
...
def cache_get(key):
    ...
    try:
        client = get_redis_client()
        value = client.get(f"demo16:cache:{key}")
        ttl = client.ttl(f"demo16:cache:{key}")
        return jsonify({
            "key": key,
