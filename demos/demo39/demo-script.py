...
"""Demo 39: Matomo Analytics.

Demonstrates deploying Matomo, an open-source web analytics platform,
...
# Demo metadata
TITLE = "Demo 39: Matomo Analytics"
DESCRIPTION = """
Demonstrates deploying Matomo with Hop3:
...
DEFAULT_HOSTNAME = "demo39.hop"
MYSQL_NAME = "demo39-db"
...
def run(ctx: DemoContext) -> None:
    ...
    # Show app structure
    print_header("Deploying Matomo Analytics")

    show_app_structure(
        APP_NAME,
        [
            ("Dockerfile", "Docker image based on matomo:5-apache"),
            ("start.sh", "Startup script with MySQL configuration"),
            ("hop3.toml", "Hop3 configuration with [[provider]] section"),
        ],
    )
    print_info("Matomo is the leading open-source web analytics platform.")
    ...
    wait_for_app(seconds=15, message="Waiting for Matomo to start...")
    check_app_status(ctx, APP_NAME)

    # Test main endpoint
    print_header("Step 5: Test Application")
    # Matomo installation page contains "Matomo" in the title
    test_app_via_curl(ctx, app_url, expected_content="Matomo")
    print_success("Matomo is running!")
    print_blank()
    ...
    cleanup_app(ctx, APP_NAME, app_url)
    print_success("Demo 39 completed: Matomo deployed successfully.")
