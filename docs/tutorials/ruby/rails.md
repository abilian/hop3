...
```bash exec id=install-rails timeout=120
...
```bash exec id=check-rails-available
rails -v
```
...
```bash exec id=create-rails-app timeout=120
...
```assert file-exists path=hop3-tuto-rails/Gemfile
```

```assert file-exists path=hop3-tuto-rails/config/application.rb
```
...
```bash exec id=add-pg-gem dir=hop3-tuto-rails
echo 'gem "pg"' >> Gemfile
```

Install the dependencies:

```bash exec id=bundle-install dir=hop3-tuto-rails timeout=120
bundle install
```
...
```bash exec id=verify-rails-structure dir=hop3-tuto-rails
ls -la
```
...
```bash exec id=generate-welcome-controller dir=hop3-tuto-rails
...
```assert file-exists path=hop3-tuto-rails/app/controllers/welcome_controller.rb
```

Create the welcome view:

```file path=hop3-tuto-rails/app/views/welcome/index.html.erb
...
```file path=hop3-tuto-rails/config/routes.rb
...
```file path=hop3-tuto-rails/app/controllers/health_controller.rb
...
```file path=hop3-tuto-rails/config/database.yml
default: &default
  adapter: sqlite3
...
```bash exec id=update-production-config dir=hop3-tuto-rails
...
```bash exec id=create-storage-dir dir=hop3-tuto-rails
...
```file path=hop3-tuto-rails/Procfile
...
```file path=hop3-tuto-rails/hop3.toml
[metadata]
id = "hop3-tuto-rails"
version = "1.0.0"
...
```file path=hop3-tuto-rails/config/puma.rb
...
```bash exec id=setup-database dir=hop3-tuto-rails
bin/rails db:create db:migrate
```

Run Rails system checks:

```bash exec id=rails-check dir=hop3-tuto-rails
...
```bash exec id=verify-final-structure dir=hop3-tuto-rails
ls -la
```
...
```file path=hop3-tuto-rails/.gitignore
...
```bash exec id=git-init dir=hop3-tuto-rails
git init
```
...
```bash exec id=git-add dir=hop3-tuto-rails
git add .
```

```bash exec id=git-commit dir=hop3-tuto-rails
