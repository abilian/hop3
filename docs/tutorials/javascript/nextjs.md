---
tutorial:
  name: nextjs-hop3-tutorial
  env:
    NODE_ENV: development
  teardown:
    - rm -rf hop3-tuto-nextjs 2>/dev/null || true
    - hop3 app destroy --app hop3-tuto-nextjs -y 2>/dev/null || true
---
...
```assert file-exists path=hop3-tuto-nextjs/next.config.ts
```

Verify the project structure:

```bash exec id=verify-structure dir=hop3-tuto-nextjs
ls -la
```
...
```file path=hop3-tuto-nextjs/next.config.mjs
...
```bash exec id=verify-config dir=hop3-tuto-nextjs
...
```bash exec id=create-api-dir dir=hop3-tuto-nextjs
mkdir -p src/app/api/health
```

```file path=hop3-tuto-nextjs/src/app/api/health/route.ts
...
```assert file-exists path=hop3-tuto-nextjs/src/app/api/health/route.ts
```

Create a simple `/up` endpoint:

```bash exec id=create-up-dir dir=hop3-tuto-nextjs
mkdir -p src/app/up
```

```file path=hop3-tuto-nextjs/src/app/up/route.ts
import { NextResponse } from "next/server";
...
```file path=hop3-tuto-nextjs/src/app/page.tsx
...
```bash exec id=verify-standalone dir=hop3-tuto-nextjs
...
```file path=hop3-tuto-nextjs/hop3.toml
[metadata]
id = "hop3-tuto-nextjs"
version = "1.0.0"
...
```bash exec id=verify-deploy-files dir=hop3-tuto-nextjs
...
```file path=hop3-tuto-nextjs/.gitignore
# Dependencies
node_modules/
/.pnp
.pnp.js
...
```bash exec id=git-init dir=hop3-tuto-nextjs
git init
```
...
```bash exec id=git-add dir=hop3-tuto-nextjs
git add .
```

```bash exec id=git-commit dir=hop3-tuto-nextjs
...
```bash exec id=deploy dir=hop3-tuto-nextjs timeout=120
...
```bash exec id=redeploy dir=hop3-tuto-nextjs timeout=120
hop3 deploy hop3-tuto-nextjs
```
