...
```assert file-exists path=hop3-tuto-axum/Cargo.toml
```

Update Cargo.toml:

```file path=hop3-tuto-axum/Cargo.toml
[package]
name = "hop3-tuto-axum"
version = "1.0.0"
edition = "2021"
...
```file path=hop3-tuto-axum/src/main.rs
use axum::{
    extract::{Path, State},
    http::StatusCode,
...
```bash exec id=build-app dir=hop3-tuto-axum timeout=300
...
```bash exec id=test-app dir=hop3-tuto-axum timeout=15
...
```file path=hop3-tuto-axum/Procfile
prebuild: cargo build --release
web: ./target/release/hop3-tuto-axum
```

```file path=hop3-tuto-axum/hop3.toml
[metadata]
id = "hop3-tuto-axum"
version = "1.0.0"
...
```file path=hop3-tuto-axum/.gitignore
# Build artifacts
target/
Cargo.lock
...
```bash exec id=git-init dir=hop3-tuto-axum
git init
```
...
```bash exec id=git-add dir=hop3-tuto-axum
git add .
```

```bash exec id=git-commit dir=hop3-tuto-axum
