...
```assert file-exists path=hop3-tuto-actix-web/Cargo.toml
```

Update Cargo.toml with dependencies:

```file path=hop3-tuto-actix-web/Cargo.toml
[package]
...
```file path=hop3-tuto-actix-web/src/main.rs
use actix_cors::Cors;
...
```bash exec id=build-app dir=hop3-tuto-actix-web timeout=300
...
```file path=hop3-tuto-actix-web/Procfile
prebuild: cargo build --release
web: ./target/release/hop3-tuto-actix-web
```

```file path=hop3-tuto-actix-web/hop3.toml
[metadata]
...
```file path=hop3-tuto-actix-web/.gitignore
# Build artifacts
target/
Cargo.lock
...
```bash exec id=git-init dir=hop3-tuto-actix-web
git init
```
...
```bash exec id=git-add dir=hop3-tuto-actix-web
git add .
```

```bash exec id=git-commit dir=hop3-tuto-actix-web
...
```bash exec id=deploy dir=hop3-tuto-actix-web timeout=120
...
```bash exec id=redeploy dir=hop3-tuto-actix-web timeout=120
hop3 deploy hop3-tuto-actix-web
```
