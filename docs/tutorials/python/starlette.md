---
tutorial:
  name: starlette-hop3-tutorial
  env:
    PYTHONDONTWRITEBYTECODE: "1"
  teardown:
    - rm -rf hop3-tuto-starlette venv 2>/dev/null || true
    - hop3 app destroy --app hop3-tuto-starlette -y 2>/dev/null || true
---
...
```assert file-exists path=hop3-tuto-starlette/venv/bin/activate
```

Install Starlette:

```bash exec id=install-starlette dir=hop3-tuto-starlette timeout=60
...
```file path=hop3-tuto-starlette/app.py
import os
import json
...
```assert file-exists path=hop3-tuto-starlette/app.py
```

## Step 3: Create Requirements

```bash exec id=freeze-requirements dir=hop3-tuto-starlette
. venv/bin/activate && pip freeze > requirements.txt
```

```bash exec id=check-requirements dir=hop3-tuto-starlette
...
```bash exec id=verify-structure dir=hop3-tuto-starlette
...
```file path=hop3-tuto-starlette/.gitignore
venv/
__pycache__/
*.pyc
.env
...
```file path=hop3-tuto-starlette/Procfile
web: uvicorn app:app --host 0.0.0.0 --port $PORT
```

```file path=hop3-tuto-starlette/hop3.toml
[metadata]
...
```bash skip
hop3 config set --app hop3-tuto-starlette SECRET_KEY=$(python3 -c "import secrets; print(secrets.token_hex(32))")
```
...
```bash exec id=deploy dir=hop3-tuto-starlette timeout=120
...
```bash exec id=redeploy dir=hop3-tuto-starlette timeout=120
hop3 deploy hop3-tuto-starlette
```
