...
```assert file-exists path=hop3-tuto-flask/venv/bin/activate
```

Activate the virtual environment and install Flask:

```bash exec id=install-flask dir=hop3-tuto-flask timeout=60
...
```file path=hop3-tuto-flask/app.py
import os
...
```assert file-exists path=hop3-tuto-flask/app.py
```
...
```bash exec id=freeze-requirements dir=hop3-tuto-flask
. venv/bin/activate && pip freeze > requirements.txt
```

```assert file-exists path=hop3-tuto-flask/requirements.txt
```

Verify the requirements:

```bash exec id=check-requirements dir=hop3-tuto-flask
...
```file path=hop3-tuto-flask/config.py
import os
...
```file path=hop3-tuto-flask/wsgi.py
import os
from app import app
...
```bash exec id=verify-structure dir=hop3-tuto-flask
...
```file path=hop3-tuto-flask/Procfile
...
```file path=hop3-tuto-flask/hop3.toml
[metadata]
id = "hop3-tuto-flask"
version = "1.0.0"
...
```bash exec id=verify-deploy-files dir=hop3-tuto-flask
...
```file path=hop3-tuto-flask/.gitignore
# Virtual environment
venv/
.venv/
ENV/
...
```bash exec id=git-init dir=hop3-tuto-flask
git init
```
...
```bash exec id=git-add dir=hop3-tuto-flask
git add .
```

```bash exec id=git-commit dir=hop3-tuto-flask
