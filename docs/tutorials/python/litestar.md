---
tutorial:
  name: litestar-hop3-tutorial
  env:
    PYTHONDONTWRITEBYTECODE: "1"
  teardown:
    - rm -rf hop3-tuto-litestar venv 2>/dev/null || true
    - hop3 app destroy --app hop3-tuto-litestar -y 2>/dev/null || true
---
...
```assert file-exists path=hop3-tuto-litestar/venv/bin/activate
```

Install Litestar:

```bash exec id=install-litestar dir=hop3-tuto-litestar timeout=60
...
```file path=hop3-tuto-litestar/app.py
import os
...
```assert file-exists path=hop3-tuto-litestar/app.py
```

## Step 3: Create Requirements

```bash exec id=freeze-requirements dir=hop3-tuto-litestar
. venv/bin/activate && pip freeze > requirements.txt
```

```bash exec id=check-requirements dir=hop3-tuto-litestar
...
```bash exec id=verify-structure dir=hop3-tuto-litestar
...
```file path=hop3-tuto-litestar/.gitignore
venv/
__pycache__/
*.pyc
.env
...
```file path=hop3-tuto-litestar/Procfile
web: uvicorn app:app --host 0.0.0.0 --port $PORT
```

```file path=hop3-tuto-litestar/hop3.toml
[metadata]
