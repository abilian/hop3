...
- **Configuration via environment.** Set values with `hop3 config set --app <app> KEY=value`. `HOST_NAME` defines the public hostname nginx serves; secrets like `SECRET_KEY` should be set this way, never committed.
