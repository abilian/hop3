...
```assert file-exists path=hop3-tuto-eve/venv/bin/activate
```

Install Eve:

```bash exec id=install-eve dir=hop3-tuto-eve timeout=60
...
```file path=hop3-tuto-eve/app.py
import os
...
```assert file-exists path=hop3-tuto-eve/app.py
```

## Step 3: Create Requirements

```bash exec id=freeze-requirements dir=hop3-tuto-eve
. venv/bin/activate && pip freeze > requirements.txt
```

```bash exec id=check-requirements dir=hop3-tuto-eve
...
```bash exec id=verify-structure dir=hop3-tuto-eve
...
```file path=hop3-tuto-eve/.gitignore
venv/
__pycache__/
*.pyc
.env
...
```file path=hop3-tuto-eve/Procfile
web: gunicorn app:app --bind 0.0.0.0:$PORT
```

```file path=hop3-tuto-eve/hop3.toml
[metadata]
id = "hop3-tuto-eve"
version = "1.0.0"
...
```bash skip
hop3 app status --app hop3-tuto-eve
...
```bash skip
hop3 app logs --app hop3-tuto-eve
