---
tutorial:
  name: laravel-hop3-tutorial
  env:
    APP_ENV: local
  teardown:
  - rm -rf hop3-tuto-laravel 2>/dev/null || true
  - hop3 app destroy --app hop3-tuto-laravel -y 2>/dev/null || true
---
...
```bash exec id=create-laravel timeout=180
composer create-project laravel/laravel hop3-tuto-laravel --no-interaction
```

```assert file-exists path=hop3-tuto-laravel/artisan
```

```assert file-exists path=hop3-tuto-laravel/composer.json
```
...
```bash exec id=verify-structure dir=hop3-tuto-laravel
ls -la
```
...
```bash exec id=generate-key dir=hop3-tuto-laravel
...
```file path=hop3-tuto-laravel/routes/web.php
<?php
...
```file path=hop3-tuto-laravel/resources/views/welcome.blade.php
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
```file path=hop3-tuto-laravel/.env.example
APP_NAME=MyApp
APP_ENV=production
APP_KEY=
APP_DEBUG=false
...
```bash exec id=update-logging dir=hop3-tuto-laravel
...
```bash exec id=create-storage dir=hop3-tuto-laravel
...
```bash exec id=laravel-check dir=hop3-tuto-laravel
...
```bash exec id=test-routes dir=hop3-tuto-laravel
...
```file path=hop3-tuto-laravel/Procfile
...
```file path=hop3-tuto-laravel/hop3.toml
[metadata]
...
```bash exec id=verify-deploy-files dir=hop3-tuto-laravel
...
```file path=hop3-tuto-laravel/.gitignore
/node_modules
/public/build
/public/hot
/public/storage
...
```bash exec id=git-init dir=hop3-tuto-laravel
git init
```
...
```bash exec id=git-add dir=hop3-tuto-laravel
git add .
```

```bash exec id=git-commit dir=hop3-tuto-laravel
...
hop3 env set --app hop3-tuto-laravel HOST_NAME=hop3-tuto-laravel.$HOP3_TEST_DOMAIN
```
...
```bash skip
# List all variables
hop3 env show --app hop3-tuto-laravel

# Set a variable
hop3 env set --app hop3-tuto-laravel MAIL_MAILER=smtp

# Remove a variable
hop3 env unset --app hop3-tuto-laravel OLD_VARIABLE
```
...
```bash skip
hop3 env set --app hop3-tuto-laravel QUEUE_CONNECTION=database
```
...
```bash skip
hop3 env set --app hop3-tuto-laravel FILESYSTEM_DISK=s3
hop3 env set --app hop3-tuto-laravel AWS_ACCESS_KEY_ID=your-key
hop3 env set --app hop3-tuto-laravel AWS_SECRET_ACCESS_KEY=your-secret
hop3 env set --app hop3-tuto-laravel AWS_DEFAULT_REGION=us-east-1
hop3 env set --app hop3-tuto-laravel AWS_BUCKET=your-bucket
```
...
```bash skip
hop3 env set --app hop3-tuto-laravel MAIL_MAILER=smtp
hop3 env set --app hop3-tuto-laravel MAIL_HOST=smtp.mailgun.org
hop3 env set --app hop3-tuto-laravel MAIL_PORT=587
hop3 env set --app hop3-tuto-laravel MAIL_USERNAME=your-username
hop3 env set --app hop3-tuto-laravel MAIL_PASSWORD=your-password
hop3 env set --app hop3-tuto-laravel MAIL_ENCRYPTION=tls
```
...
Common issues:
- **Missing APP_KEY**: It's generated automatically on the first deploy via the `[env]` `{ generate = ... }` declaration; check `hop3 env show --app hop3-tuto-laravel` to confirm it's set
...
```bash skip
hop3 env set --app hop3-tuto-laravel COMPOSER_MEMORY_LIMIT=-1
```
...
```bash skip
hop3 env show --app hop3-tuto-laravel | grep DATABASE
```
