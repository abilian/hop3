...
- **Secrets are config, not files.** Set `APP_KEY` (Laravel) or `APP_SECRET` (Symfony) with `hop3 config set` — keep them out of git. Set `APP_ENV=production`/`prod` and `APP_DEBUG=false`/`0` the same way.
- **Logs go to stderr.** Point your framework's log channel at stderr (`LOG_CHANNEL=stderr` for Laravel) so `hop3 logs` captures them.
