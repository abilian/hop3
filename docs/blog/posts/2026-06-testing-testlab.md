...
A nightly result is only trustworthy if it doesn't depend on what ran before it. So a Test Lab run starts from a blank slate: for cloud targets, a full **OS rebuild of the throwaway server**, the Hop3 install included. A run that *silently* tests a dirty server is worse than no run at all. The constraint therefore has sharp edges:
