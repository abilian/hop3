...
**Hop3 0.6.1** is out, two days after [0.6.0](2026-06-hop3-060-release.md). 0.6.0 added platform capability (resource limits, a richer addon surface, a signed catalog); 0.6.1 is a **consolidation release**: it simplifies how you point the CLI at a deployment, makes Nix builds reproducible, pulls the email addon forward, and works through a broad stability pass.
...
A **context** is now a deploy environment: `devel`, `staging`, and so on, declared in your committed `hop3.toml` under `[contexts.<name>]`: a non-secret bundle of server address, app, domains, and env. `--context` is the one selector; the `--server` flag, the `hop3 server` command, and `servers.toml` are gone. Credentials drop out of sight into a per-server credential store (`~/.config/hop3-cli/credentials.toml`, mode `0600`), and `config.toml` becomes secret-free: local preferences and an optional default-context pointer. Existing connections migrate to the credential store on first run. When a context can't be resolved, the CLI surfaces the full resolution chain; it no longer falls back silently to a default server.
...
- The **Test Lab** itself reports more truthfully: a run that completes with failing tests is recorded as *failed* where it was previously marked *crashed*; the report shows the packaging variant (native / docker / nix) and each demo under its real name; dispatch runs off-thread with quieter scheduler logs; and the queue view gained build numbers, timestamps, and a foldable detail.
