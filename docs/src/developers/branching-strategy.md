...
*   **Current Version:** Work-in-progress towards the next release.
...
*   **Branch from:** `devel`
*   **Merge to:** `main` (for the official release) and `devel` (to incorporate any stabilization fixes).
*   **Naming Convention:** `release/vX.Y` (e.g., `release/v0.5`).
*   **Workflow:**
    ...
    3.  Once stable, the release branch is merged into `main` and tagged with the version number (e.g., `v0.5.0`).
...
*   **Branch from:** `main`
*   **Merge to:** `main` and `devel`.
*   **Naming Convention:** `hotfix/vX.Y.Z` (e.g., `hotfix/v0.5.1`).
*   **Workflow:**
    1.  A `hotfix/` branch is created from the `main` branch.
    2.  The critical bug is fixed and committed.
    3.  The branch is merged back into `main` and tagged with a new patch version.
...
*   **Users:** Use the `main` branch for a reliable installation.
*   **Maintainers:** Manage the release cycle by creating `release` branches from `devel` and merging them into `main` and back into `devel`. Urgent production fixes are handled with `hotfix` branches.
