...
"""Setup dnsmasq for wildcard DNS resolution of *.hop3.dev on macOS.

This script configures dnsmasq to resolve all *.hop3.dev addresses
...
def setup_dnsmasq(server_ip: str) -> bool:
    ...
    # Create hop3.dev wildcard config (for remote testing)
    # and hop3-docker.dev (for Docker testing)
    print(f"\nCreating {hop3_conf}...")
...
Examples:
  ...
  %(prog)s hop3.dev          # Resolve hostname to IP
  %(prog)s myserver.example.com
""",
    )
    parser.add_argument(
        "server",
        help="Server IP address or hostname to resolve *.hop3.dev to",
    )
