...
**Status:** partial, covering the 20-application benchmark corpus. **Updated:** 2026-07-30.
**Source:** `notes/benchmarks/2026-07-28-matrix.jsonl`, a blank-slated four-variant run. Tiers from `hop3-tools nix tiers`.
**What M2.3 requires:** a version declaration over the Nix runtime, whose deliverable is a *stated disposition per application*. A milestone requiring everything to be green invites either a long tail of marginal work or an unannounced redefinition of the corpus; a milestone requiring every application to have a recorded disposition can be finished, and can be checked by someone else.
...
- **The corpus beyond the benchmark set.** There are 31 generated recipes and 31 hand-written ones; this run exercised 20 applications. The remaining recipes have no recorded disposition, and that is the open part of M2.3.
- **`forgejo` and `easy-appointments`**, named as red in earlier notes and absent from this corpus. `forgejo`'s reported symptom was a 180-second health-check timeout, which is the exact signature of the closure guard failing to fire. Its disposition should not be decided before that guard is confirmed working, or the wrong defect gets fixed.
...
An application's disposition is one of **runs**, **fix** (ours, with the defect named), **defer-upstream** (theirs, with the blocker named), or **drop** (with the reason). "Not tested recently" is not a disposition; it is the absence of one, and it is recorded as pending.
