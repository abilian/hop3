...
- **Related-ADRs**: [002](./002-config-format.md) (hop3.toml format), [003](./003-config-parsing-and-validation.md) (config validation), [011](./011-encryption.md) (encryption), [016](./016-backups.md) (backup strategy), [024](./024-backup-restore-system.md) (backup/restore system), [035](./035-build-artifacts.md) (build artifacts), [040](./040-network-firewall-and-port-exposure.md) (network/firewall/ports), [041](./041-privileged-operations-agent.md) (privileged operations agent), [042](./042-cli-context-model.md) (CLI context model), [045](./045-fixed-port-registry.md) (fixed-port registry)

## Abstract

This ADR establishes a declarative model for application resources in `hop3.toml`: the app declares intent and the platform realizes it idempotently, failing loud when it cannot. It specifies five config surfaces (typed `[env]` values (generated secrets and dynamic references), `[[volumes]]` persistence, `[limits]` resource caps, resource-aware `[backup]` policy, and proxied secondary `[port]` endpoints) and consolidates deploy-time ignore patterns into `[build].ignore`. Realizations that cross the kernel privilege boundary (mounts, cgroups) run through the hop3-rootd agent ([ADR 041](./041-privileged-operations-agent.md)) behind a single op contract.

## Context

Hop3's `hop3.toml` was modeled on Nua's `nua-config` (Hop3's predecessor). [ADR 002](./002-config-format.md) reserved a number of Nua-derived fields against future implementation, but several were never realized, and a side-by-side comparison of the two formats surfaced four capabilities that Nua expressed declaratively and Hop3 cannot. Each one forces a *per-app workaround*: which, by the project ethos, is a signal that the platform is missing something.
...
This ADR is a single **umbrella** decision: it sets the direction and design principles for the whole declarative-resource surface, specifies the four capabilities above, **folds in and extends** two overlapping areas (backups → [ADR 016](./016-backups.md)/024; multi-port proxying → [ADR 040](./040-network-firewall-and-port-exposure.md)), and defers the remainder (source-acquisition-by-config, descriptive metadata, addon version pinning) to *Future Work*.
...
Left unaddressed, the advertised set stays fragile and order-dependent, the silent-drop defect keeps lying to users about their config, and the gap between the documented format ([ADR 002](./002-config-format.md)) and the runtime widens.
...
4. Folded-in: `[backup]` **per-resource policy** extending [ADR 024](./024-backup-restore-system.md) (superseding the backup-config sketch in [ADR 002](./002-config-format.md) §backup); `[port]` **proxied secondary endpoints** extending [ADR 040](./040-network-firewall-and-port-exposure.md).
5. Consolidate **deploy ignore patterns** into `[build].ignore`, removing the `.hop3ignore` sidecar and the `[build].ignore-file` pointer.
6. Realize the privilege-crossing parts (kernel mounts, cgroups) through the hop3-rootd agent under one op contract ([ADR 041](./041-privileged-operations-agent.md)).
...
- **`tmpfs`**: a sized RAM mount at `src/<target>` (a real kernel mount), for caches/scratch. `size` is **required** and format-validated like `[limits].memory`: an uncapped tmpfs defaults to half of RAM, a multi-tenant footgun, so a sizeless tmpfs aborts at schema time; a cross-section validator rejects `Σ tmpfs size ≥ [limits].memory`. It is never seeded (scratch; shadowed shipped content is *logged*) and **never backed up** (a `[volumes.backup]` on a tmpfs is itself a config error).
- **`bind`**: an operator-approved host path, declared with a `source` field (absolute host path (the deliberate inverse of the relative `target`)). Binding arbitrary host paths is a host-escape risk, so this is **default-deny**: only paths under an operator allow-list (`HOP3_BIND_VOLUME_ALLOWLIST`, empty by default) are accepted, with `source` `realpath`-resolved and re-checked against the resolved allow-list in *both* hop3-server and rootd (no symlink escape). Two apps binding the same source is the unmanaged-shared-resource hazard, so a `BindClaim` registry (mirroring `PortClaim`) detects contention and aborts. Backups **default-exclude** bind sources (operator-owned, possibly shared/huge); opt in with `[volumes.backup] include = true`. Destroy leaves the source in place: the bytes live at `source` outside `app_path`, so once unmounted the mountpoint is an empty directory. A *still-mounted* bind would make `rmtree(app_path)` follow into operator data, which the teardown gate below prevents.
...
#### 4a. Backups: extend [ADR 024](./024-backup-restore-system.md), supersede [ADR 002](./002-config-format.md) §backup
...
Hop3 routes HTTP by hostname on a dynamic `$PORT`; raw non-HTTP ports use `[[ports]]` ([ADR 040](./040-network-firewall-and-port-exposure.md)/045). The missing case is a *second proxied HTTP(S) endpoint* (e.g. an admin UI on a different container port):
...
- New realization paths (volume linking, cgroup wiring) with OS variance, several depending on hop3-rootd ([ADR 041](./041-privileged-operations-agent.md)): a hard dependency for limits and bind/privileged mounts.
...
- **Secrets**: CSPRNG only (`secrets`); stored encrypted at rest once [ADR 011](./011-encryption.md) lands; never logged except a `display = true` one-shot to deploy output (documented, opt-in).
...
- The `from`/`key`/`random` `[env]` forms (documented but never implemented) are now defined and interpreted. [ADR 016](./016-backups.md) and [ADR 024](./024-backup-restore-system.md) extended for resource-aware backup policy; [ADR 040](./040-network-firewall-and-port-exposure.md) extended for proxied secondary endpoints. No deployed app breaks.
...
- [ADR 002](./002-config-format.md), [ADR 003](./003-config-parsing-and-validation.md), [ADR 011](./011-encryption.md), [ADR 016](./016-backups.md), [ADR 024](./024-backup-restore-system.md), [ADR 035](./035-build-artifacts.md), [ADR 040](./040-network-firewall-and-port-exposure.md), [ADR 041](./041-privileged-operations-agent.md), [ADR 042](./042-cli-context-model.md), [ADR 045](./045-fixed-port-registry.md).
