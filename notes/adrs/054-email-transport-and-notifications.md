...
- **Deliverability is gated by DNS and IP reputation the platform does not own**, so it must be reported truthfully, never implied.
- **Some apps read no SMTP environment** (stock WordPress, PHP `mail()`, cron) and must be pointed at a submission endpoint through their own configuration, not through injected variables.
