...
*Motivation*: `create` / `destroy` carry semantic weight that makes them inappropriate for entities existing outside Hop3 ("destroying a user" sounds violent; users are people Hop3 registers).
...
- **Plugin aliases**: registered via the plugin manifest (see [ADR 039](./039-python-deploy-strategies.md)).
...
#### D18: Plugin extension: see [ADR 039](./039-python-deploy-strategies.md)

Plugins register commands, sub-namespaces, top-level namespaces, and aliases. Full design is [ADR 039](./039-python-deploy-strategies.md); key decisions captured in [`plugin-mechanism-todo.md`](../cli/plugin-mechanism-todo.md):
...
**(c) Mutating commands acknowledge state change.** Every command that mutates server state prints a one- or two-line summary of what changed, to stderr (per rule (a)). Silent success on mutation is a bug.
...
| Item | Where |
|------|-------|
| Plugin extension mechanism | [ADR 039](./039-python-deploy-strategies.md) |
| Blueprint commands | Deferred until Blueprint feature is designed ([ADR 031](./031-project-terminology.md)) |
...
- [ADR 039](./039-python-deploy-strategies.md): Plugin CLI Extension (brief at [`plugin-mechanism-todo.md`](../cli/plugin-mechanism-todo.md))
