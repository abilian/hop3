...
| `retired` | Withdrawn for a reason upstream of the platform — the project is archived, the licence is incompatible, another variant supersedes it. | no |
...
A recipe's status is the directory containing it, and nothing else records it. A `status = "beta"` field beside a `beta/` directory would be a second source of truth for one fact, and the failure mode of that arrangement is well attested here: the two drift, the file wins in one code path and the field in another, and neither is wrong enough to fail.
...
- An operator can be shown what is proven, what is provisional, and what is known broken, instead of a single undifferentiated list.
...
- `hop3-tools` acquires the promotion commands ADR 057 anticipated: moving a recipe between statuses is a tool's job, because it must re-run the admitting check rather than move a directory.
...
- A status can go backwards. A `golden` recipe whose next verification fails is demoted, and demotion is the normal outcome of a failing run rather than an exception to be argued with.
