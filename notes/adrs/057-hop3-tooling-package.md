...
The boundary is the **audience**, not the mechanism:

- **Belongs in `hop3-tooling`:** work the people who maintain Hop3 and its catalog run — repo, release, and ops automation.
...
- Maintainer tooling gains discovery (`hop3-tools --help`), a UX contract, tests, and a shared library that removes duplication — the same properties every shipped package already has.
...
- **Fold the tooling into `hop3-testing`.** Overloads a package whose job is the test framework. A catalog promoter or a version-bumper is not a test; mixing them muddies the one boundary that package needs to keep sharp. Rejected. (`hop3-tools` calling `hop3-testing` is fine — that is a dependency, not a merge.)
- **Makefile targets.** A `Makefile` is a fine thin entry point *into* tools, but not where non-trivial logic, `--help`, or tests belong. Kept as a complement, not a substitute.
...
When packaging decouples from platform evolution and the catalog grows maturity channels (`unstable`/`testing`/`stable`), `hop3-tools` is the natural home for channel management (promoting an app across channels). Those commands are added when the channels exist, not now. The package name is settled (`hop3-tooling`); the command name (`hop3-tools` vs. alternatives) can settle in review.
