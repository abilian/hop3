...
- **Operator confirmation (ADR 041 §9).** This code path opens the firewall rule directly inside `do_deploy`, without the delta-confirmation ADR 041 envisions for privileged operations (the existing nginx-reload path is likewise direct). The reserved-port guard prevents the worst case (claiming 22/80/443); a general "show the privileged changes, confirm before applying" step across all rootd-driven deploy actions is a separate, cross-cutting follow-up, not unique to this feature.
