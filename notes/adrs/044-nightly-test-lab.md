...
- **Type**: Architecture
- **Created**: 2026-06-05
- **Related-ADRs**: [043](./043-unified-testing-architecture.md) (unified-testing-architecture), [041](./041-privileged-operations-agent.md) (privileged-operations-agent), [042](./042-cli-context-model.md) (cli-context-model), [034](./034-streaming-deployment-logs.md) (streaming-deployment-logs), [033](./033-docker-integration.md) (docker-integration), [018](./018-cli-architecture.md) (cli-architecture)
## Context
...
[ADR 043](./043-unified-testing-architecture.md) makes the nightly data *exist and be uniform* (the shared `collect_diagnostic_bundle`); this ADR builds the product that makes it *visible, queryable, and scheduled*. The two are complementary.
...
- `collect_diagnostic_bundle(target, test) -> Bundle`: [ADR 043](./043-unified-testing-architecture.md)'s shared collector, the data source for §C. **This ADR depends on [ADR 043](./043-unified-testing-architecture.md)'s diagnostic bundle.**
...
This is the heart of the product: [ADR 043](./043-unified-testing-architecture.md)'s bundle made first-class and browsable. For **every** test, pass or fail, the runner captures the bundle (build / app / nginx / journal / proxy-probe / deploy / http / dns / manifest) *before teardown*, since the logs vanish with the server. The lab-specific design rules make the data *useful*:
...
- The 6 h budget becomes a managed dial (pool size).

**Negative / costs**

- A new subproject to build and maintain (web + scheduler + worker + schema + UI).
- Hard dependency on [ADR 043](./043-unified-testing-architecture.md)'s diagnostic bundle: the product is only as good as the bundle.
...
- The dashboard exposes deployment internals → require authentication (reuse Hop3 auth / [ADR 014](./014-authentication-bootstrap.md)); not public by default.
...
- **Static report only (pytest-html / Allure / `generate_html_report`).** No scheduling, trigger, trends, scaling, or live fill. Kept as an export.
...
6. **Auth & exposure**: own credentials for v1; reuse Hop3 auth ([ADR 014](./014-authentication-bootstrap.md)) later.
7. **Relationship to SourceHut**: SourceHut stays the per-distro PR/commit CI ([ADR 043](./043-unified-testing-architecture.md)); the Lab owns the nightly full-suite surface.

## Future work

- Notifications (email/Matrix/…) on new regressions.
- **Schedule the reproducibility check.** `hop3-tools nix check-reproducible` rebuilds every nix-gen recipe and compares the output against the first build ([ADR 058](./058-build-reproducibility-model.md)). It is the half of the advertised gate that no schedule currently drives, so the claim it defends is only ever as fresh as the last time someone ran it by hand. The Lab is its natural owner: it already provisions, holds credentials, and stores per-app results, and a rebuild verdict is the same shape as a test result.

  Two constraints distinguish it from the app suites. It must **not** run inside the blank-slate nightly: that run rebuilds the OS, so every check would start from a cold `/nix/store` and cost hours. It also has no use for the blank slate, since `nix build --rebuild` compares against its *own* first build; a warm store only makes it faster, and a store carried across days strengthens the comparison. It therefore wants a separate schedule against a persistent box.
