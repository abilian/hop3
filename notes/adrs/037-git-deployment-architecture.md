...
Both `git-receive-pack` and `git-upload-pack` (for clones/fetches) are wired. The `hop3 app:deploy` path remains available alongside `git push`; they are alternatives, not replacements.
