...
**Trust-budget change (the load-bearing cost).** §10 *describes* a hardened unit (`CapabilityBoundingSet=CAP_NET_ADMIN`, `ProtectControlGroups=true`, `PrivateMounts=true`), but the unit the installer actually ships runs minimal/un-hardened pending the v0.6 hardening redesign — the heavy sandbox caused `203/EXEC` crash loops with rootd's nft/nginx/systemctl executor role (`server_installer/rootd.py`, and the regression test `test_service_template_is_minimal_pending_v06_hardening`). On that minimal unit, cgroup writes and host-namespace mounts **already work today**, so Phase 2 needs *no* unit loosening to ship. The directives below are instead the **forward-constraint** the eventual v0.6 hardening must satisfy, threat-modelled as one change rather than two:

- writing the cgroup hierarchy → keep `ProtectControlGroups` false/unset (or systemd-delegate `hop3.slice` via `Delegate=`), with `ReadWritePaths` scoped to exactly that subtree;
- mounting into the app's namespace → keep `CAP_SYS_ADMIN` and the host mount namespace (no `PrivateMounts`; `MountFlags=shared`), because a mount made in rootd's *private* namespace is invisible to the Emperor-spawned app process — a "mounted successfully" report over an empty dir, i.e. silent success.

When that hardening is designed, this is a materially larger kernel surface than CAP_NET_ADMIN-only; it is accepted only with the scoping above and the §1 framing (a hop3-server compromise is already total compromise of the hop3 user — rootd's job is to not *widen* that). The minimal-unit regression test now also guards these ops from a premature `ProtectControlGroups` / `PrivateMounts` re-add. Shipping now: an installer-time cgroup-v2 host check and a default-deny bind allow-list file (`/var/lib/hop3-rootd/bind-allowlist`); the bind allow-list is the real control on `mount.bind` and must stay conservative.
