...
- **Type**: Feature
- **Created**: 2024-07-17
- **Related-ADRs**: [002](./002-config-format.md), [003](./003-config-parsing-and-validation.md)
...
Schema validation beyond TOML parse errors is owned by [ADR 003](./003-config-parsing-and-validation.md). A dataclass plus `@property` model catches structural errors at access time; load-time schema validation, together with a `hop3 validate` command for CI gating, builds on that model.
