...
Nix's guarantee is a *property of the build*. A Nix-derived binary and a Dockerfile-built binary may behave identically at runtime. The guarantee matters for audit, upgrade, and supply-chain attestation. It does not change how the application behaves at run time. The guarantee has a cost, and applications whose upstream build system assumes unrestricted network access are the ones that pay it. Most PHP, Python, Node and Ruby packaging resolves dependencies during the build, and cannot be built inside Nix's strict sandbox without accommodation. Tier-2 (§3.4) exists for exactly this case: `__noChroot`-derivations sacrifice some of the guarantee in exchange for the ability to package a much wider application ecosystem. The guarantee is a gradient. Hop3's three-tier taxonomy makes that gradient explicit.
...
The plugin architecture is intentionally narrow: a small number of orthogonal protocols cover every extension point. The decomposition emerged from repeated refactoring as the application catalogue grew. Each protocol corresponds to a point in the deployment pipeline where the behaviour demonstrably differs across applications, and where forcing a single implementation into the core would have required per-application `if`-ladders.
...
A small number of applications are **explicitly deferred** with documented blockers. **Monica** hits a Laravel Mix / webpack 5 incompatibility, resolved upstream only in Monica v5, which moves to Vite. **SonarQube** bundles an Elasticsearch that crashes without kernel-level `vm.max_map_count` tuning, outside the PaaS scope. The **Vaultwarden** docker and native variants are blocked upstream: only Docker images are shipped, so a source build would exceed the Docker builder timeout and require a Rust toolchain on the target. Each deferral carries a per-app `DEFERRED.md` under `apps/bad/` listing the blocker and the unblocker, so a third party can verify the gap or close it.
...
Each variant carries its tier in the template metadata (§3.5), so an auditor reads a deployed application's reproducibility guarantee off its packaging.
...
| **Tier-1/2/3** | The reproducibility taxonomy for Nix-packaged applications, *as of April 2026*: fully sandboxed (1), `__noChroot` escape (2), prebuilt artefact (3). Superseded: see the note at the head of this report. Tier 2 is now a hermetic source build, so the tiers rank provenance. |
