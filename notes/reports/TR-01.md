...
> **Superseded material (noted 2026-07-22).** This report describes the system as of April 2026 and is left as that record. One part of it has since been overtaken: the reproducibility taxonomy in §3.5 defines Tier-2 as a `__noChroot` escape that grants the build network access, and §6.3 defers dependency vendoring as too costly to be worth it. Vendoring was subsequently implemented for all six ecosystems, so `__noChroot` no longer appears in any template, every template builds offline in the sandbox against a lockfile-pinned dependency set, and the tiers now classify provenance rather than hermeticity. See [ADR 008](https://git.sr.ht/~sfermigier/hop3/tree/main/item/notes/adrs/008-nix-builders-2.md) for the current taxonomy and the final report for the measured results. Read §3.5 and §6.3 as a statement of where the work stood, not of where it stands.
...
The claims of this report are primarily qualitative and architectural. A quantitative evaluation (§5.4, §7) — control-plane footprint against K3s, Docker Compose and bare uWSGI; deployment latency; Nix closure versus Docker image size — is planned for the final report later in 2026.
...
Three observations follow. First, Nix's guarantee is a *property of the build*, not of the running artefact; a Nix-derived binary and a Dockerfile-built binary may behave identically at runtime. The guarantee matters for audit, for upgrade, for supply-chain attestation, not for operational concerns. Second, the guarantee has a cost: applications whose upstream build system assumes unrestricted network access — most PHP, Python, Node, Ruby packaging, whose package managers resolve dependencies during the build — cannot be built inside Nix's strict sandbox without accommodation. This is what motivates Tier-2 (§3.4): `__noChroot`-derivations sacrifice some of the guarantee in exchange for the ability to package a much wider application ecosystem. Third, the guarantee is a gradient rather than a boolean. Hop3's three-tier taxonomy makes that gradient explicit rather than papering over it with marketing.
...
The addon subsystem provides a uniform surface across heterogeneous services. Each addon implements three operations — provision (create the resources, emit connection parameters), destroy (remove them), and env (produce the environment variables the application expects) — and is responsible for managing its own state.
