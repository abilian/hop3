...
**Failure taxonomy.** Four of the 80 cells failed, and their causes separate cleanly into platform defects and harness artefacts rather than a single reliability figure. `native/bugsink` was rejected by the platform's own pinning gate (*"has unpinned requirements"*) — a genuine recipe defect, and the gate behaving correctly. `nix/bugsink` and `nix-gen/gitea` both exceeded their configured start timeout (270 s and 299 s), which is a configuration bound rather than a demonstrated inability to start. `native/wordpress` returned HTTP 200 but failed a content assertion, because a freshly-deployed WordPress serves its installation wizard rather than the expected default post — a defect in the test's expectation, not in the deployment. Counting only the platform defect, 79 of 80 cells behaved correctly; we report the raw 72/80 as well, since which of these one counts as a failure is a judgement the reader should be able to make.
...
The remaining cells — the second-instance (warm-cache) timing, the version-to-version update deltas measured across two releases, the Tier-2/Tier-3 reproducibility outcomes, the memory-versus-application-count curve, and a Dokku baseline — are the subject of the accompanying measurement release. The harness (`hop3-bench`) and the raw measurements are part of the public artifact (§9).
