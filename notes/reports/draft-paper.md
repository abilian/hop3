...
**Result.** All twenty catalog applications install from the signed catalog and accept a real login: BookStack, Bugsink, Dolibarr, Easy!Appointments, Forgejo, Gitea, Invoice Ninja, Isso, Kanboard, Keycloak, LimeSurvey, Matomo, Mattermost, Miniflux, Nextcloud, Paheko, Radicale, Uptime Kuma, Vikunja and WordPress. This includes **Matomo** and **Dolibarr**, which an earlier snapshot of this report deferred as browser-wizard-only. The most recent complete recorded run, on a server rebuilt from a blank operating system the same evening, returned **19 of 20** in 24 minutes, with per-application times from 35 s to 352 s. The failure is worth reporting precisely, because it is the kind this report argues for surfacing rather than smoothing: Radicale's install aborted after 8 seconds because the platform ran the application's declared build command against the system Python interpreter rather than the virtual environment it had just created for it, and the distribution's Python refuses installation into itself. That is a platform defect, since fixed, and not a property of the application; the fix is not yet reflected in a completed run, so the figure stands at 19 of 20 rather than being rounded up. A run with all twenty green has not yet been recorded.
