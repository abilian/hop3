...
**Scope of the claims.** Every claim about the build path here is checkable by running something, and §6 names the check alongside the claim. The templating step receives no formal characterisation, and none is relied upon.
...
**Deploy cost across strategies (measured).** We ran the full four-variant matrix over the 20-application golden set, running 80 cells on a single blank-slated server. Each application was deployed and then verified through its HTTP surface before teardown. This is the first like-for-like deploy-cost comparison across the three build strategies; timings are wall-clock from `deploy` to a verified response, and only successful cells contribute to the statistics.
...
The caveats attached to individual results are collected here.
...
**Construct validity of "deployed".** A deployment is counted as successful when the application answers over HTTP and the response contains an application-specific marker. That is stronger than a status code and weaker than exercising the application's features. For the golden set the bar is raised to an authenticated interaction (§6.1); for the wider corpus it is not.
...
Work on Hop3 repeatedly ran into gaps in the surrounding tooling, and several of the resulting tools became artefacts in their own right. Some of the claims in this report are checkable because a tool was written to check them. [TODO: links to the the projects we describe, as references]
...
**Evaluation infrastructure.** `cloudalone` provides a single-box substitute for a cloud provider's API, written when the availability of cloud instances became a practical constraint on running the measurement protocol of §6.4.
...
`validoc`, `md2typst`, `tepyd` and `mcpython` are published on PyPI and are declared development dependencies of this repository, so the checks described above are reproducible from a checkout.
