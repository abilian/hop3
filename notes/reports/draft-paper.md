...
Mainstream application orchestration has converged on container clusters built around distributed consensus stores [TODO: add references here]. For single-server deployments (common in small-to-medium organisations, at the network edge, and wherever digital sovereignty rules out external control planes) the overhead of this paradigm is disproportionate to the problem [XXX: is this really the issue?]. We present **Hop3**, an open-source Platform-as-a-Service designed for autonomous single-server operation [XXX: single-server is only the starting point]. Hop3 provides a Heroku-style developer experience (git-push deployment, automatic language toolchains, managed backing services) without requiring Docker or Kubernetes, running on commodity hardware with a small control-plane footprint. The system is structured around a decoupled control plane (JSON-RPC over SSH, Litestar ASGI, local SQLite or PostgreSQL state, Fernet-encrypted secrets) and a plugin-driven build and deployment pipeline supporting ten [TODO: check that’s indeed 10] language toolchains. An integration with the Nix package manager [10] provides hermetic builds with eleven parametrisable templates that generate `hop3.nix` expressions from a declarative `hop3.toml` spec; every template builds offline in the Nix sandbox against a dependency set vendored from a committed lockfile, and 31 of 31 template-generated recipes rebuild bit-for-bit identically, 30 of them still doing so when the pinned nixpkgs revision is moved forward by a minor release [XXX: 31 and 30 are anecdotal numbers]. We state four design requirements (determinism, bounded overhead, autonomy, encrypted secrets), describe the architecture against these requirements, and report a 157-variant application corpus drawn from 58 distinct upstream projects across Docker, native, hand-crafted-Nix and template-generated-Nix deployment strategies, of which a curated 20-application catalog is additionally verified through each application's authenticated interface — a bar the platform now enforces on every deployment, and which exposed twenty-three platform defects invisible to a deployment that merely returned HTTP 200 [XXX: again, 157, 58 and 20 are aribitrary or anecdotal]. Preliminary measurements over this set quantify the design: a blank server reaches a running, verified application in about nine minutes [XXX: on a specific server; also: is it good how bad? nobody knows]; the control plane holds 185 MB, 7.8× lighter than a lean K3s carrying the same workload [TODO: check this is true]; and Nix closure sizes, cross-application deduplication (21–36%) and update deltas are measured against the equivalent upstream Docker images [TODO: check also]. The paper closes with a discussion of two extension paths: a richer multi-service application model (per ADR 038) for apps such as Mastodon and AppFlowy that exceed the current single-process-tree assumption, and a longer-term direction toward multi-node operation along the lines reviewed by Vaño et al. [21] for edge-native deployments.
...
4. A 157-variant application corpus, drawn from 58 upstream projects, packaged across the four build strategies and deployed end-to-end on a remote VPS, with per-variant diagnostic logs automatically collected on failure. Packaged and verified counts are reported separately (§6.1).
...
| Hand-crafted Nix (`hop3.nix` written by operator) | 34 | Explicit dependency control |
| Template-generated Nix (from `hop3.toml`) | 31 | Eleven templates (see §5.4); auto-derived from a lockfile-equivalent manifest |
| **Total variants** | **157** | Drawn from 58 distinct upstream applications |
...
**Two sets of twenty, chosen for different purposes.** This report uses two curated twenty-application sets, and they are not the same set. The **catalog set** is the collection published in Hop3's signed application catalog and advertised as production-ready; it was chosen for operator utility and packaging feasibility. The **benchmark set** (§6.3, §6.4) was chosen for *language balance* (PHP 5 / Go 5 / Python 4 / Node 3 / JVM 3), so that cross-strategy timings are not dominated by one ecosystem. Twelve applications belong to both. Where this report says "the golden set" without qualification the intended set is named; the functional-verification result below concerns the catalog set, and the deploy-cost and closure measurements concern the benchmark set. Twenty is the number this project undertook to deliver rather than a property of the design, and the catalog is a living collection: its membership has already changed once (Gatus was replaced by Uptime Kuma) and is expected to grow. Both sets are therefore reported as snapshots, dated, rather than as fixed corpora.
...
**Deploy cost across strategies (measured).** We ran the full four-variant matrix over the 20-application benchmark set (§6.1), running 80 cells on a single blank-slated server. Each application was deployed and then verified through its HTTP surface before teardown. This is the first like-for-like deploy-cost comparison across the three build strategies; timings are wall-clock from `deploy` to a verified response, and only successful cells contribute to the statistics.
...
*Table 4: Deploy time and coverage by build strategy, the 20 benchmark-set applications × 4 variants (n=1 per cell).*

The result runs against the common expectation that Nix dominates deploy cost. Both Nix paths land within 12–18% of the native toolchain (110 s and 116 s against 98 s) and are **1.4–1.5× faster than Docker** at the median, with native 1.7× faster; Docker's mean is worse than its median (194 s) and carries a long tail (521 s for Vikunja, 398 s for Directus), because each application's image is constructed from a base image on every deploy, whereas a Nix deploy materialises paths that a warm store already holds. Docker is nonetheless the only strategy with complete coverage (20/20); the Nix paths account for all four missing recipes, which is itself the cost of the Nix path: hand-written closures do not exist for every application.
...
**Evaluation gap.** The measurements in §6.4 come from a preliminary run whose limits are statistical: single samples, no spread, a two-point stand-in for the memory-versus-count curve, and baselines drawn from container orchestration rather than from the single-server PaaS systems §2.3 names as Hop3's peers. §7.4 states these and the other threats together. Completing the suite means repeated samples with confidence intervals, the memory-versus-count curve, second-instance measurements, and those peer baselines.
...
Vaño et al.'s review [21] organises the edge-orchestration field around two axes: lightweight K8s distributions (K3s, MicroK8s) versus K8s-adapted-for-edge frameworks (KubeEdge, OpenYurt, SuperEdge, Open Horizon, Baetyl). Both axes presuppose Kubernetes as the substrate. Hop3 sits *off* this axis altogether: it provides a PaaS-level interface without Kubernetes and without containers as a hard requirement. We do not claim this is a strict improvement over the Kubernetes-derived path. For multi-node, high-availability, multi-tenant deployments it would not be. We claim it is a better fit for a large class of real workloads (single-server SMB and sovereignty-focused deployments) that the K8s-derived path serves with substantial overhead. Section 6.1 documents a 157-variant packaged corpus, of which the 65 Nix variants require no container runtime at all; the most recent full Nix run deployed and content-checked 40 of them on a remote host.

### 7.4 Threats to Validity

**Sampling.** Every figure in §6.4, and every cell of Table 4, is a single sample. No repeats, no spread, no confidence intervals. Medians across twenty applications are reported because the corpus is wide, and that width does not substitute for repetition: a slow disk or a warm cache on one run moves a cell and nothing would reveal it.

**Ordering.** The four variants of each application ran sequentially on one host, always in the same order. Later variants therefore inherit whatever the earlier ones left in the page cache, the Nix store and the Docker layer cache. The direction of the bias is knowable but its size is not, and it plausibly flatters the strategies that run later. Randomising the order within each cell would isolate it; that has not been done.
...
**Construct validity of "deployed".** A deployment is counted as successful when the application answers over HTTP and the response contains an application-specific marker. That is stronger than a status code and weaker than exercising the application's features. For the catalog set the bar is raised to an authenticated interaction (§6.1); for the wider corpus it is not. §6.1 also shows the gap between those two bars is not small: every catalog application cleared the weaker bar while a majority carried a defect that made logging in impossible. Any count in this report resting on the weaker bar should be read with that in mind, and the 157-variant corpus figure of §6.1 rests on it.

**Two twenty-application sets.** The catalog set (§6.1) and the benchmark set (§6.3, §6.4) share twelve members and differ in eight, so a result established on one does not transfer to the other: the functional verification of §6.1 does not cover the eight benchmark-only applications, and the deploy-cost and closure measurements do not cover the eight catalog-only ones. Each figure names its set. Both are snapshots: twenty is the number this project undertook to deliver, the catalog has already changed membership once, and it is expected to grow, so any count here should be read as of its stated date rather than as a fixed property of the system.
...
Work on Hop3 repeatedly ran into gaps in the surrounding tooling, and several of the resulting tools became artefacts in their own right. Some of the claims in this report are checkable because a tool was written to check them. Two ties to the funding should be distinguished: a tool was either **funded by this project** as a deliverable in its own right, or built alongside it to ship one. [TODO: links to the the projects we describe, as references]
...
| **T4** Packaged applications | M4.1–M4.4 — 20 applications in four batches, each with experience reports | Yes | §6.1, §10.1 | 157 variants from 58 upstream projects; a 20-application catalog verified through authenticated login; 20 experience reports plus an aggregate |
