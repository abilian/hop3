...
| | M2.3 Final release ("1.0") | Yes | §5, §6.1, §6.2 | The Nix runtime ships in 0.7.0, this project's final release, exercised corpus-wide rather than on a sample: 62 Nix variants run on it, every catalog application signs in under both Nix strategies, and 30/30 template-generated recipes rebuild bit-for-bit. Hop3 versions as a whole, so "1.0" names the maturity the annex asked for rather than a component version string |
...
| | M3.8 Outcomes of the security audit and accessibility scan | Yes | §7.4, §9.1 | Four in-house review rounds processed and every finding remediated, after a third-party reviewer was applied for and followed up twice without one being allocated; the security model published across three pages. The accessibility scan was not carried out and its effort went to the fourth security round — the substitution is stated rather than glossed |
...
| | M5.6 Videos / screencasts | Yes | §9.1 | 68 asciicasts covering every demo and tutorial in the corpus, recorded by driving the real scripts and published with a per-file account of what each contains. A first pass, and a qualified one: 11 ran to completion, 33 end on a visible failure and 24 recorded nothing, from two defects in the recorder rather than in what it records. Both are fixed; the re-record follows |

*Table 5: the funded project's milestones and where each is evidenced.* Every milestone is delivered. Three carry a qualification in the table: M2.3's "1.0" names the maturity the annex asked for rather than a component version string, since Hop3 versions as a whole; M3.8's accessibility scan was not carried out, its effort having gone to a fourth security round after no third-party reviewer was allocated; and M5.6's recordings cover every demo and tutorial but only 11 of the 68 ran clean. Where a milestone is not fully met this report says so in the row rather than in a footnote, and §7.4 carries the corresponding limitations.
...
**Dissemination (M5.1, M5.2, M5.4, M5.6).** The project site currently carries 25 posts. The documentation runs to 215 pages for developers, administrators and end users, including 46 tutorial pages across twelve language and framework stacks; the tutorials are executed against a live server as tests (§8), so a tutorial that stops working fails a test run. The work was presented at OSXP 2025 and at OW2con in 2025 and 2026. 68 terminal screencasts cover every demo and tutorial in the corpus and are published with the recordings themselves; the review pass found that only 11 ran to completion, 33 end on a visible failure and 24 recorded nothing, and the cause in both cases was the recorder rather than the software being recorded — a fixed 120-second step timeout that 30 tutorials hit at their deploy step, and a manifest that reported success for a file it had written without reading. They are published as they are, with the state of each stated, rather than trimmed to the eleven that look finished; the re-record is outstanding.
