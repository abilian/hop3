...
**Hop3** is an open-source Platform-as-a-Service for a single server. It gives a developer the deployment loop the Twelve-Factor movement made an industry standard [6]: `git push`, automatic language detection and build, managed backing services, TLS and a hostname, rollback; all on a machine the operator owns and administers. Its design centre is a server running a handful of long-lived applications that has to keep working without an external control plane. That is the ordinary situation for a small or medium organisation, for a node at the network edge, and wherever digital sovereignty [16], [17] rules out a control plane somebody else operates. Single-node is the starting point; the architecture admits multi-node deployment (§9), but every claim made here is for the single-node case.

The control plane is a single ASGI process (JSON-RPC over SSH, a Litestar core, local SQLite or PostgreSQL state, Fernet-encrypted secrets) with no distributed consensus store to keep quorate. It drives a plugin pipeline factored along two independent axes, three build strategies and ten language toolchains, so integration cost grows additively across the two axes. An integration with the Nix package manager [10] supplies a hermetic build path: eleven parametrisable templates generate `hop3.nix` expressions from a declarative `hop3.toml` spec, each sealing its ecosystem's package manager behind a dependency set vendored from a committed lockfile and building offline in the Nix sandbox.

Two results carry the evaluation. Every template-generated recipe in the corpus rebuilds bit-for-bit identically: a census of the full set and the report's strongest empirical claim. Both Nix build paths deploy **1.4–1.6× faster than Docker** and within 18% of a native toolchain build, across two independent runs of a four-strategy matrix, against the usual expectation that hermeticity is paid for in deploy time. The control plane holds 185 MB where a lean K3s carrying the same workload on identical hardware holds 1441 MB; closure sizes, cross-application deduplication (21–36%) and update deltas are measured against the equivalent upstream Docker images.

The application corpus is an instrument. 58 upstream projects packaged across four build strategies exist to exercise the platform's edges; they also yielded a methodological finding. A curated subset is verified by **signing in** through each application's own authentication, a bar the platform enforces on every deployment. Adopting it exposed twenty-three platform defects invisible to a deployment returning 200. Applied afterwards to a variant family that a deploy-oriented instrument had just scored uniformly passing, the same bar **failed all sixteen** of its applications, five of which were serving with no usable credential at all; the same corpus reached 16 of 16 within a day of being measured.
...
- [6. Evaluation](#6-evaluation)
  - [6.1 Application Coverage](#61-application-coverage)
    - [6.1.1 The Packaged Corpus](#611-the-packaged-corpus)
    - [6.1.2 Functional Verification of the Catalog Set](#612-functional-verification-of-the-catalog-set)
    - [6.1.3 The Same Bar, Applied to the Nix Families](#613-the-same-bar-applied-to-the-nix-families)
...
- **Sovereignty:** the operator holds the data, the configuration and the keys. No component phones home or requires a control plane it does not run [16], [17]. §7.5 gives this as a set of technical invariants that can be checked.
- **Reproducibility:** hermetic builds through Nix, so that the deployed artefact is a function of what is committed [8], [10].

The mainstream answer to deployment is by now a container cluster, with Kubernetes the de facto standard [4]. Its central mechanisms (a consensus-based control plane, a container runtime, a service mesh) exist to schedule work across a fleet of individually unreliable machines, and where a fleet exists they earn their keep. Hop3 is aimed at the case where no fleet exists, so the distinction is one of problem shape before it is one of size. Absent a fleet, those mechanisms persist along with their costs: resident memory [5], [20], a container supply chain that must be trusted and kept current, and operational expertise. Resident memory is the one of the three that can be measured cleanly, and §6.4 measures it.
...
The PaaS model, pioneered by Heroku and formalised in the Twelve-Factor App methodology [6], provides a developer-centric deployment interface: push source code, and the platform handles building, running and scaling. Cloud Foundry and OpenShift extended this to enterprise contexts [7].

**Dokku** adapts the model to a single server by combining shell plugin scripts with Docker: a git push triggers a buildpack or `Dockerfile` build, the result runs as a container, and backing services are provided by plugins. It holds no persistent control-plane process, keeping state in files and container labels, which makes its resident footprint close to that of the Docker daemon it drives.

**Piku** takes the opposite decision on containers, running applications as ordinary processes under uWSGI behind nginx, with no container runtime at all; it is distributed as a single script and was written for hardware as small as a Raspberry Pi. Hop3 originates from Piku and retains code from it, so the process-level deployment model described in this report is inherited work; Hop3 adds the plugin decomposition of §4.3, the Nix build path of §5, and the addon and diagnostics layers. Neither system has a peer-reviewed description, and both codebases serve as their own reference.

Two more recent entrants occupy the same single-server niche. **CapRover** wraps a web GUI around Docker Swarm, which introduces distributed-systems dependencies even for single-node use. **Coolify** offers a polished Vercel-style self-hosted experience, Docker-based, with no reproducible-build story.

Hop3 occupies the same design space and differs from these systems in four places: (i) the build pipeline is factored into builder × language-toolchain, exposing no buildpacks directly; (ii) Nix integration gives an alternative build strategy with stronger hermeticity than the native buildpack/toolchain path; (iii) a single uniform plugin architecture spans builders, toolchains, addons and proxies; and (iv) per-variant deployment diagnostics are collected automatically on failure. Table 1 summarises the comparison.
...
Nix and containers are not alternatives. nixpkgs can emit OCI images directly (`dockerTools.buildImage`, `buildLayeredImage`, and third-party tools such as `nix2container`), so a reproducibly-built image is entirely achievable, deployable by any platform that deploys images. Reproducible builds are therefore available to Hop3's peers, in the sense that an operator can build that way and hand the result over.

Hop3 treats the closure as the deployment unit. The store is materialised on the host and the application runs as an ordinary process, so no container runtime is required at run time and no registry is required at deploy time. This has two measurable consequences, reported in §6.4. The store is shared across every application on the host, which yields cross-application deduplication. An update transfers only the store paths that changed, leaving image layers out of the exchange. Layered Nix-built images recover part of the first effect at layer granularity when layers are identical; we did not measure that variant, and do not claim a margin over it.
...
Recipes pin nixpkgs to a single commit (nixos-24.11), with one exception: an application that entered nixpkgs later can override the pin for itself, which `etherpad` does (a nixos-25.05 revision), because the package does not exist in the default. The corpus is therefore evaluated against *two* pins, and "the pinned nixpkgs" below means that pair. The override is per-application, validated at parse time (a malformed revision is refused), and available to any recipe.

The distribution over the template-generated recipes is 6 Tier-1, 23 Tier-2, 2 Tier-3. It moved substantially during the project: applications initially packaged as Tier-3 wrappers were migrated to source builds as the vendoring pattern reached each ecosystem. The two that remain are those whose upstream ships no buildable source for the packaged version; nixpkgs itself packages these from the upstream artefact. Each migration was a one-line template change in `hop3.toml`. The two generic prebuilt templates are retained with no current consumer: they remain the fast path for packaging an application without lockfile work and the only path for proprietary software distributed as a binary, for which no source build is possible in principle.
...
Treating the closure as the deployment unit (§5.1) carries a liability that the image-based alternative does not. It follows from the same property that makes the approach attractive. An image is held alive by the container runtime that references it; reclaiming it while a container runs is not a state the runtime permits. A Nix closure is ordinary host state, referenced by absolute path from a process Nix does not supervise, with nothing in the Nix model keeping it alive. A garbage collection (invoked by an operator, a distribution's periodic timer, or a disk-pressure hook) is free to reclaim files that a running application is executing. The failure is delayed and unlike its cause: the next time the process is launched, it dies `No such file or directory` on a path that was present when the application was deployed. In a supervised setting this surfaces as a start-up timeout naming nothing.
...
Retention and detection were verified separately, at different levels of evidence. The distinction governs how much the mechanism is worth, since only one of them prevents anything. Detection is verified by an injected fault on a provisioned target: deleting a path from a deployed application's live closure and then restarting the application produces the named abort. Retention is verified by construction, without fault injection: the prior closure is observably registered as a root across a rebuild-while-running. No run has yet demonstrated that a collection then leaves both closures intact, because the container used for the other measurements cannot complete a collection (Nix scans `/proc` for runtime roots and aborts on a read the sandbox refuses). Establishing it requires a target whose Nix can collect, and is outstanding.

The verification itself produced a second, more general result. The detector's decision logic was thoroughly unit-tested, yet fault injection revealed it was not being reached on the very sequence it was written for: a defect invisible to any test of the mechanism, because it concerns whether the mechanism runs at all. It is fixed. The injected fault now produces the named abort. A guard is two claims: that it decides correctly, and that it runs; testing the first is routine, while testing the second requires constructing the fault.

## 6. Evaluation

This section reports three results. Every template-generated Nix recipe in the corpus rebuilds bit-for-bit identically (§6.2, with the requirements evidence for R1–R4). Both Nix build paths deploy faster than Docker and close to a native toolchain build, replicated across two independent runs (§6.3; the benchmark protocol and the remaining measurements are in §6.4). Verifying an application by signing in changes the answer: §6.1 reports one corpus moving from uniformly passing to uniformly failing when the bar was raised.

§7.4 states where each claim's replication stops.

### 6.1 Application Coverage

#### 6.1.1 The Packaged Corpus
...
**What raising the bar exposed.** Every application in the catalog set deployed successfully and served HTTP 200 *before* this verification existed. Bringing each of the twenty to a real login required twenty-three fixes. Their distribution is the finding: the defects were almost entirely in the **platform**, not in the application recipes. Among them:

- Applications were served over plain HTTP as well as TLS, so the `Secure` cookies that most of them set were never returned by the browser, and every such login looped silently.
- PHP's built-in server was run with a single worker, so any application issuing a sub-request to itself during installation deadlocked against its own only worker; ten of the twenty run on that server.
- An installation whose deployment failed could not be retried: the application record was committed before the deployment was attempted, so every subsequent attempt was refused as a duplicate.
- A declared build command was honoured by five of the twelve language toolchains and silently dropped by the rest.
- A clean reinstall reclaimed the platform's records of its backing-service databases without reclaiming the databases, so the next installation adopted a populated one.

None of these is visible to a deployment that returns 200, and none was specific to an application.

**Verifying an application whose password you no longer hold.** A check that authenticates as the administrator stops working the moment an operator changes the administrator's password, which is both expected and desirable. Reporting such an application as unverified would be accurate but useless, and storing the operator's new password is not acceptable. Hop3's answer is an optional **probe account**: a non-privileged account created through the application's own user-management interface, whose credential the platform owns and rotates, used by the check in preference to the administrator's. It is declined per application where the deployment's threat model does not tolerate a platform-held account. Eleven of the twenty catalog applications declare one, all eleven exercised against live instances; the remainder fall back to the administrator credential. Every check reports which account it used.

#### 6.1.3 The Same Bar, Applied to the Nix Families

The catalog set is packaged in three build strategies, and until recently only the native one had been verified this way. Applying the sign-in bar to the other two produced complete recorded runs of **18 of 19** template-generated variants and **16 of 16** hand-crafted ones. A subsequent whole-corpus run (2026-08-01) added the result the evaluation had been missing: the native family, the strategy the catalog actually publishes, at **20 of 20** on a single recorded run. The three families stand at 20/20, 18/19 and 16/16.

The run also produced an instructive failure. One hand-crafted recipe stopped installing. Its diagnosis is a caution about instrument design. It presented as a start-timeout, and had been read as one on previous occurrences (three separate increases to the timeout had been made across the three variants of that application). It was not slow: its web server bound the port eight seconds in and forked a worker that never booted, hanging silently for the remaining 232 seconds of the window. The platform had emitted the correct diagnosis every time, directly beneath a headline that said "failed to start within N seconds". Operators acted on the headline. The underlying defect was that the recipe ran a two-process application as one, its setup being confined to the web process's start wrapper and therefore unshareable with the second worker. Corrected, the application passes in 40 seconds.

Three days before the first of those runs, a deploy-oriented matrix had scored the hand-crafted recipes in its scope 15 ok and 0 fail. The sign-in bar, on the same corpus, **failed all sixteen**. Five were serving with no usable credential at all: a calendar server whose authentication type defaulted to `none` and was never set, so every calendar and address book was public; a comment moderator whose configuration omitted its administrator section, so the moderation dashboard was served disabled; two applications (one of them an identity provider) shipping a literal `changeme`, so the deployed instance had an administrator whose password is in the repository while the operator held one that did not work; and a survey tool whose installer ran with a hard-coded password and discarded its own exit status. No status assertion and no content assertion can distinguish any of those from a working deployment. Every instrument the project then had reported them working.

This does not mean the recipes were poor: the same corpus reached 16 of 16 within a day of being measured. The 0/16 measures the distance between "deploys" and "works" on a corpus where every instrument had agreed for months.
...
Two of these compounded. Making the toolchains honour the declared build command was correct and immediately broke three applications, because the honoured command then ran against the system interpreter. The command was never executed, so the environment it would have run in was irrelevant, and the first defect concealed the second for as long as it lasted. Repairing a configuration key that was silently inert is therefore not a safe change: it introduces a new code path, not a completion of an old one. The conformance test proving the key was now honoured everywhere did not catch it; a deployment run did.

### 6.2 Requirements Evidence from the Test Harness

The test harness provides direct evidence for each of R1–R4:

- **R1 (Determinism, Nix path):** every template-generated recipe rebuilds bit-for-bit identically, 30 of 30 in the corpus at the time of the audit, with no sampling. Each recipe is built and then rebuilt with `nix build --rebuild`, comparing the second output against the first, on a freshly-provisioned x86_64 host with nixpkgs pinned at `50ab793` (nixos-24.11). The first audit of the same corpus returned 25 of 30. Every closed gap was fixed in the *template*, so recipes added since inherit the property: build-verification timestamps and prune markers stripped from the pnpm store, `dontFixup` to stop stdenv writing an interpreter store-path into vendored scripts, `RECORD` rewriting in the Python wheel set, strict validation in the composer FOD. Because every template vendors its dependency set and builds offline, the property holds across all three tiers, so determinism does not distinguish them; the tiers classify provenance instead (§5.3).

  **What the guarantee is relative to.** The property is relative to one pinned nixpkgs revision, which has to move for security updates. One pin is shared by the whole corpus, so a single recipe that fails against a new revision holds back the security updates of the other thirty. Failures also correlate by ecosystem: a change to a fetcher or a language hook meets every recipe using it at once. Twenty-three of the thirty-one recipes are Tier-2, which places their lockfiles and vendor hashes in our hands. This is the structural trade the Nix path makes: integration work that a distribution performs once for all its users is transferred to whoever owns the pin.
...
The matrix was run twice, a week apart, each time on a freshly rebuilt host, and two variants were additionally re-run on their own in between. Each *cell* remains a single sample within a run, so no per-cell confidence interval is available. The repetition establishes that the per-variant medians and the ordering between them survive an independent run on a clean machine.
...
The result runs against the common expectation that Nix dominates deploy cost. Both Nix paths sit within 18% of the native toolchain and are **1.4–1.6× faster than Docker** at the median, in both runs. In run B the hand-written Nix path was in fact the fastest of the four (101 s against 106 s native). That is not evidence of Nix beating a native build, since the two are within the spread of the native medians across runs (98–106 s), but it does bound how large any Nix penalty can be: on this corpus it is not distinguishable from zero.

Docker's disadvantage is stable and structural. Its mean is far worse than its median (198 s against 166 s), with a long tail (528 s for Vikunja, 398 s for Directus): an image is constructed from a base image on every deploy, whereas a Nix deploy materialises store paths that a warm store already holds. Docker is nonetheless the only strategy with complete coverage in both runs. The Nix paths account for every missing recipe: a hand-written closure does not exist for every application, and in run B five were absent.

The one direction of travel visible between the runs is coverage: `nix-gen` went from 19/20 to **20/20** after the packaging defect described below was fixed, while `native` lost a cell. Median deploy times moved by 4–8% in both directions; that is the scale of run-to-run variation to assume for every timing in this report.
...
A preliminary run gives the following measured figures. Closure sizes and per-process memory are from the dev deployment (8 vCPU, 16 GB, Linux 6.8). The build-install timings and the baseline comparison are from freshly-provisioned cpx41 boxes of the same class (8 vCPU, 16 GB, x86_64). The nixpkgs pin is nixos-24.11, with one exception noted in §5.4.
...
There is no universal size winner. The Nix closure is *larger* than a minimal (scratch/Alpine) upstream image (dramatically so for the static-Go apps and for Keycloak, which ships the full JDK as store paths), but *smaller* than a fat upstream image: Mattermost's closure is 246 MB against a 425 MB image. Per-application size tracks how minimal the upstream image is; the packaging model has little bearing on it. The Nix disk advantages that hold regardless are **cross-application deduplication** and reproducibility. Deduplication depends on runtime homogeneity: the union closure saves **36%** across the four applications sharing a common runtime graph (Miniflux, Gitea, Forgejo and Vikunja; their shared paths are glibc, git and bash) and **21%** across all six, the Keycloak and Mattermost closures sharing little with the rest. The saving grows with the number of co-hosted applications sharing a runtime. The **update delta** (the application's own store path, re-sent on a source-only bump) is 19–164 MB, below the full closure. For applications whose upstream image is already minimal, though, it is comparable to, or larger than, re-pulling that image, so the bandwidth advantage of the Nix delta (§5.3) is real only where the runtime graph is large and shared. A reproducibility check confirms the R1 story for this subset: rebuilding each of the six from source (`nix build --rebuild`) yields a byte-identical `narHash`.

**On re-taking these figures.** Re-measured on 2026-07-28, all eighteen cells returned identical values. This confirms the recipes and the pipeline have not drifted; it is not evidence against the sampling criticism of §7.4, since a closure size, a store-path count and an update delta are deterministic functions of a pinned recipe and revision. The timings and the memory figures carry the run-to-run variance; the missing repeats matter there.

**Build-and-install time.** On a freshly-provisioned x86_64 VPS (Hetzner cpx41, 8 vCPU, 16 GB), a blank server reaches a running, HTTP-verified application in **528 s (≈ 9 min)** end to end, unattended. This is a reference point. We are aware of no published equivalent for the comparable systems of §2.3, so there is nothing to be faster or slower than. The figure's value is that a third party can re-take it on the same instance class and compare. It bounds the cost of the property that matters operationally: the path from a blank operating system to a verified application is a single unattended command, so a node is reconstructible (S1, §7.5). This covers operating-system dependencies, all language toolchains, the Hop3 control plane, and the first application (Radicale), fully automated. With the platform and toolchains already installed, deploying a further application takes **131–177 s**, largely independently of the build strategy: native Go (Gitea 131 s, Miniflux 160 s), native Python (Radicale 173 s), Nix-generated (Forgejo 139 s) and Docker (Gitea 146 s, Isso 177 s) all fall in the same band. The build itself is not the bottleneck for these applications. The fixed pipeline cost (dependency reconciliation, health verification and teardown) dominates, so the choice of builder barely moves the total. (Each per-application figure is a full deploy-verify-teardown cycle and therefore bounds the deploy cost from above.)
...
**Control-plane footprint versus the baselines.** The comparison rests on two metrics with different jobs. Proportional set size (PSS) is the figure we report for Hop3 itself, because it does not charge shared page cache to the process and is stable against the machine's history: 166 MB on a freshly rebuilt box, 186 MB on the long-lived development host, 205 MB with one application deployed. The cross-stack comparison instead uses the systemd-service cgroup's `memory.current`, one metric applied identically to every stack. That metric charges page cache, which makes it strongly history-dependent: the same Hop3 cgroup read **142 MB** minutes after an operating-system rebuild and **1139 MB** after a day of building and deploying, of which `memory.stat` attributes **914 MB to page cache** and only 159 MB to the processes themselves. It is usable as a bounded comparator between stacks measured under like conditions, not as a quantity.

On that comparator:

| Stack | Idle | One workload |
|-------|-----:|-------------:|
| Docker Compose (`dockerd`) | 27 MB | 65 MB |
| Hop3 (`hop3-server` + `hop3-rootd`) | — | 185 MB |
| Lean K3s (v1.36.2; Traefik, servicelb and metrics-server disabled) | 1183 MB | 1441 MB |

*Table 8: control-plane memory by stack (systemd-service cgroup `memory.current`), with one workload deployed where applicable.*

Like for like, Hop3's control plane is **7.8× lighter than K3s** with one workload deployed (185 MB against 1441 MB), and 6.4× lighter than an idle K3s; both K3s figures exceed the 500+ MB reported by Koziolek & Eskandani [20]. K3s additionally consumes ~916 MB of whole-system RAM as reported by `free`, a figure that includes the kernel and base OS; we compute no ratio from it, since no matching whole-system measurement of a Hop3 box was taken. The comparison carries two caveats. The Docker Compose and K3s figures come from freshly-provisioned cpx41 boxes while the Hop3 figure comes from the development host of the same class (8 vCPU, 16 GB), an asymmetry that runs against Hop3 (§7.4). The baselines' cache decomposition was also not recorded, so settling the multiplier means re-taking all three stacks with `memory.stat` recorded. The durable claim is the ordering: a consensus-based control plane costs roughly an order of magnitude more than a single-server one.
...
**Maturity of the Nix runtime.** The Nix *build* path is better evidenced than the Nix *runtime* path. The build path carries the reproducibility census (§6.2). The runtime path (a generated runtime description driving the process supervisor, with the closure-lifetime machinery of §5.5) ships in 0.7.0: the contract works, applications of real complexity run under it, it is protected by an automated gate, its detection of a reclaimed closure verified by fault injection on a target. §9.8 lists what remains open; nothing in §5 or §6 depends on it, because every measurement reported here concerns builds.

**The generator's reach is narrower than the platform's.** Almost every template packages software *fetched from elsewhere*: a release tarball, a registry package, a nixpkgs attribute, an upstream binary. The evaluation corpus consists of such software, which shaped the template set. An operator deploying their own application, the git-push case the platform otherwise centres on, is a different shape: the source is already present. Two templates support it today (`go-source`, `ruby-bundler`, the latter by construction); for the rest, a first-party application can reach the Nix path only through a hand-written expression, which is the barrier the generator exists to remove. The gap is a consequence of validating against third-party software: the corpus never exercised the case, so the templates never grew it. The limitation belongs to the present system. The change is mechanical (build the recipe directory instead of a fetched archive) and does not affect the reproducibility argument, since the dependency-pinning machinery is unchanged either way.

**Evaluation status.** The measurements are preliminary and unevenly so; §7.4 states the limits, and §9.8 the work that would close them.
...
- Under the Nix build path, an application update transfers only the changed store paths, leaving the rest in place; §6.4 measures this delta for the Tier-1 subset.
...
**Provisioning asymmetry.** The control-plane memory comparison places Hop3 on the project's long-lived development host and K3s and Docker Compose on freshly-provisioned instances of the same class. A long-lived host accumulates page cache, so the asymmetry runs against Hop3 and the reported ratio is a lower bound. It remains an asymmetry, one that a re-measurement on a fresh host would remove.
...
**Construct validity: "deployed" is not "works".** A deployment is counted as successful when the application answers over HTTP and the response contains an application-specific marker (stronger than a status code, weaker than exercising the application's features). For the catalog set the bar is raised to an authenticated interaction (§6.1); for the wider corpus it is not, so any count resting on the weaker bar (including the 154-variant corpus figure) should be read with that in mind. The distance between the two bars is measured: §6.1 reports the two-instrument comparison in which a corpus uniformly green at the weaker bar uniformly failed the stronger one, and reached 16 of 16 within a day. The datum is a *maximum* for the gap, since that corpus had never been measured at the stronger bar before; and the stronger bar has its own ceiling, since a defect such as open registration passes it cleanly.
...
**Two twenty-application sets.** The catalog set (§6.1) and the benchmark set (§6.3, §6.4) share twelve members, so a result established on one does not automatically transfer to the other; each figure names its set. The two are being merged into a single corpus, so this is a property of the snapshot; every count here should be read as of its stated date.

**Maturity of the verification mechanism.** The check scripts and the deploy-time gate are new at the time of writing. Every declared probe account has now run against a live instance (the variant generator grafts an application's probe declaration into its Nix variants; both Nix families were exercised end to end), so the mechanism is demonstrated, though only on recent runs. The driver that exercises the full catalog loop is a fourth test entry point alongside the three enumerated in §8.3; folding it into the harness is planned work.
...
A report that describes a finished system tends to imply the effort was distributed the way its sections are. It was not. The distribution can be examined through two records, kept for different reasons: the 58 numbered **decision records** extant at the analysis cut-off, written before or alongside the work they govern; and a set of **lessons-learned notes**, written afterwards when something had cost more than it should have. The first records what was decided; the second records what surprised us. They do not agree. The disagreement is the interesting part.
...
*Table 9: design effort by area. The shares are deliberately coarse. Word counts are a weak proxy (decision records differ in verbosity, several carry long rejected-alternatives sections and a terse record can govern more implementation than a discursive one), so only the broad masses are meaningful; no ordering within a band should be inferred. Commit counts are an independent and equally imperfect proxy, given for the paths that principally serve each area. Security, for example, is dense in design text and light in commits against the privileged helper, because that work moved trust boundaries.

Three observations survive the coarseness. Each contradicts an expectation held at the start.

**The two records rank the same work oppositely.** Nix is a modest line in the decision record and the *largest single file* in the lessons-learned notes. The command surface is the reverse: the largest area by design text and commits, and one of the smallest lesson files. The pattern is consistent and it names two different kinds of cost. Nix was cheap to decide and expensive to learn: its model was settled early and correctly. The expense landed in per-ecosystem particulars that no amount of design would have anticipated: which package manager writes a timestamp into its lockfile, which build hook rewrites an interpreter path into a vendored script, which fixed-output derivation hashes stably. The command surface was the opposite. Its implementation was never the hard part. Deciding what it should do was, three times over.

**Integrating a correct idea is cheap; the expense lands elsewhere.** The Nix figure above is a consequence of the property §5.3 describes. Nix's model provides the guarantees. The remaining work was to find one vendoring pattern per ecosystem and express it as a template, after which it generalises mechanically: a new application on an existing template inherits reproducibility without further design. The corollary is that effort concentrated where no external model was available to inherit.

**The command surface was the largest and most-revised area.** Seven decision records, some 22,000 lines across the client and the server-side handlers, and 464 commits. Its design was iterated in the open. ADR 025 improved the user experience. ADR 036 replaced that with a full ergonomics model. ADR 052 replaced parts of that again, to give every command-line tool one flag lexicon. ADR 042 reworked the context model on top. ADR 047, which would settle how the resolved application and environment travel with each invocation, is still a draft.

The reason is that a command surface has no acceptance test. A build either yields a working artefact or does not; a rebuild either matches or does not. An interface is correct when the obvious command does the obvious thing. What counts as obvious depends on the user's working directory, their previous command, and which of several plausible mental models they hold. Each design was defensible on paper; only use could show where it needed to change. Work remains: the chain resolving which application a bare command targets is hard to reason about, several concepts remain reachable under more than one spelling, and two governing records are still drafts (§9.5).

**Security grew to about a quarter of the design record, largely unplanned.** The funded plan named firewalls and an audit. What the work produced was twelve decision records including the largest in the project (ADR 041, which redesigned privilege so that the control plane never holds root), plus a firewall design superseded once (040 → 045), secret storage consolidated after two sources were found to disagree, generated application credentials, and a per-application uid separation proposal still open. Most of it was reactive: each round of review moved a trust boundary, and moving a boundary in a platform that installs software as root is not a local change. It is an instance of the pattern §10 states as the transferable result.

Two quieter observations shaped the schedule as much as any of these. Test infrastructure became a product in its own right: `hop3-testing` is 19,681 lines, the second-largest package after the server itself, and test code across the repository is comparable in size to the source it covers. A whole class of effort also leaves almost no trace in the decision record (process supervision, database-connectivity particulars across native and containerised deployment, cross-distribution parity, asynchronous-to-thread boundaries), because it consists of discovery. Half the lessons-learned corpus is about that class. A reader reconstructing this project from its ADRs alone would substantially underestimate it.
...
The "Described in" column routes each milestone to wherever this report treats it. Some are treated at length by an earlier section because the evaluation turns on them; the rest are described in §8.1.1, so that every milestone has an account in the text as well as a row in the table.
...
*Table 10: the funded project's milestones and where each is evidenced.* Every milestone is delivered; the M2.3 and M3.8 rows carry their own qualifications, and §7.4 carries the corresponding limitations.

#### 8.1.1 Milestone Detail

The milestones below are the ones no earlier section already covers. Each is documented at length in the repository's decision records.

**Backing services and email (M3.1).** Applications declare backing services in their configuration and the platform provisions, wires and encrypts credentials for them (§4.3, R4). Email is treated as a backing service with a swappable backend. The operator selects a backend once at the server level: a provider relay, a development sink that captures and never sends, or direct delivery in which the node is its own mail transfer agent. An application opts in by attaching an email addon. The application-facing contract is a loopback SMTP endpoint that does not change when the backend does, so the provider credential never enters an application's environment. Direct delivery ships as a preview: it fails loudly where it cannot run, with its deliverability caveats documented.

**Upgrades and backups (M3.2, M3.3).** An application upgrade snapshots, redeploys, runs migrations, verifies health, and restores the snapshot automatically if any of those steps fails. Backups are cross-instance: a backup taken on one node restores onto another and redeploys there (the operation that makes a Hop3 node replaceable), verified end to end including the negative paths (collision, corrupted manifest). A server upgrade confirms the control plane came back before reporting success.
...
Work on Hop3 repeatedly ran into gaps in the surrounding tooling. Several of the resulting tools became artefacts in their own right and are useful outside Hop3. Several of the claims in this report are checkable only because a tool was written to check them. Those published on PyPI are cited at their package index entry [25]; the remainder are in the project's public repositories.

**Security.** `LeWAF` is a web application firewall written in Python: a SecLang parser, the OWASP Core Rule Set, integrations for the common Python web frameworks, and a standalone reverse-proxy mode. It occupies two positions at once. It is an independent library, published under Apache-2.0 and usable as middleware by any Python web application; and it is the engine behind Hop3's own layer-7 firewall (ADR 050). Choosing it over the more widely-cited Go and C engines was deliberate. It drops into Hop3's existing Python process model with no additional build or runtime story. It is also a codebase this project controls, so a rule-engine gap is a bug we can fix. That second reason is the sovereignty argument of §7.5 applied to our own supply chain; we found it persuasive enough to write an engine.

**Verification and test quality.** `forall`, a model checker for Python, is used to establish a property that ordinary tests can only sample. Hop3 validates application names in two places, the server and the privileged helper (§4.2), where a divergence between them would be a security-relevant defect. The parity of the two validators is expressed as a proof obligation that runs as a property test under `pytest` and as an exhaustive check under `forall check`, so the two gates are shown to accept the same language at every input length. `tepyd`, the "test pyramid doctor", assesses the shape of a test suite against declared tier budgets and is configured per package in this repository. `llm-sec-audit` and the associated `letscode` plugins [29] apply language models to security review, and were used during the audit rounds described in §7.4.
...
**The public catalog.** The signed catalog whose verification §6.1 reports also has a public face: `apps.hop3.cloud`, a browsable site presenting each published application (the catalog set and its Nix variants, 55 entries at the time of writing) with description, stack, resource requirements and category (Figure 3). It is rendered by `hop3-marketplace`, a static-site generator in this repository that reads the catalog through the same loader and taxonomy code as the server's dashboard. The site is itself deployed as a Hop3 static application. Not among the funded deliverables, the catalog grew out of the packaging campaign as the one place the catalog set is curated, signed and installed from; it is the artefact `check-catalog.py` (§8.3) drives.

![The public application catalog at apps.hop3.cloud](figures/apps-hop3-cloud.png)

*Figure 3: the public catalog at `apps.hop3.cloud`. The site is generated by `hop3-marketplace` from the same catalog data the platform installs from, and is itself deployed as a Hop3 static application.*

**Design-space exploration.** `punix` is an experiment in software deployment along the lines of Nix, Guix and SlapOS, built on an explicit theoretical foundation, and was trialled during this project as an alternative to the Nix path described in §5. Its existence reflects a view this report also takes: the guarantees in that section follow from the underlying deployment model, so a different implementation of the same model would inherit them. `nanopython` is a reimplementation of MicroPython in Zig, complete enough to run `hop3-rootd`. `situ` is a user-interface framework for Python web applications in which each piece of state declares where it lives (browser, URL, or server) and a Python-to-JavaScript compiler derives the transport, so the client/server boundary is checked at compile time and no client JavaScript is written by hand. A branch uses it to rebuild the server's dashboard declaratively, resolving interaction issues the current HTMX layer wires by hand; it is planned for the 0.8 release.
...
`punix`, `nanopython` and `situ` are research experiments produced by the funded work. None ships in Hop3 today; no release status is asserted for them.

### 8.3 Artefact and Data Availability

Hop3 is free software under the Apache-2.0 licence. The source, the plugin pipeline and the full application corpus (`apps/real-apps-native`, `apps/real-apps-docker`, `apps/real-apps-nix`, `apps/real-apps-nix-gen`) are public at https://github.com/abilian/hop3. The measurement harness used in §6.4 ships in the same repository (`hop3-bench`, in the `hop3-tooling` package). The benchmark protocol and the raw measurement data are under `notes/benchmarks/`, so every figure in §6.4 traces to the run that produced it and can be re-taken with a single command. The application-coverage figures in §6.1 are reproducible from a pinned commit/tag.

**Verification instruments.** Reproducing the claims of §6 means running four entry points, each with its own job. `pytest` covers the in-process layers (unit and integration) and a Docker-backed end-to-end layer. `hop3-test` provisions a target, deploys the application corpus to it and verifies each over HTTP; it is the instrument behind the coverage figures. `validoc` executes the documentation (§8.2). `check-catalog.py`, in the catalog repository, drives the operator-facing catalog loop described in §6.1 (enumerate, install, authenticate, destroy) and was written during the verification campaign because none of the other three exercised the path an operator actually takes from the published catalog to a working application. It is small and deliberately outside the platform: it uses only the public command-line interface, so it tests what a user has. That independence is why it found what it found. It is also why it duplicates target management and result reporting that `hop3-test` already implements. Folding it into the harness while preserving its outside-in stance is outstanding work.

## 9. Future Work

The system described here is the single-node case brought to a working state. This section sets out where it goes next. The ordering follows one architectural change. Turning the privileged helper into a node agent makes most of the rest possible; several items that look independent are consequences of it. Two entries differ in kind: §9.5 concerns a part of the system that is finished in the sense of working and unfinished in the sense of being right, and §9.8 concerns the evaluation. Where a direction already has a design record, that record is cited; several are still drafts and are identified as such.

### 9.1 From a privileged helper to a node agent

Hop3 already contains most of the mechanism a distributed deployment needs, in a component built for an unrelated reason. The privileged helper (`hop3-rootd`, ADR 041) exists so that the control plane can run unprivileged: it exposes a small set of *typed* operations over a local socket, authenticates its caller by kernel-supplied process credentials, and refuses anything outside its vocabulary. The control plane asks a separate component to execute privileged work, across a narrow, explicit interface.

That interface is the one a remote agent needs. Replacing its local socket with an authenticated remote transport turns the helper into a **node agent** and leaves the control plane as an **orchestrator** which no longer runs on every machine it manages. Nothing in the operation vocabulary changes; only the party on the other end of it does. The design work is therefore mostly about identity, transport and reconciliation.

The intended model is not the imperative one. ADR 017 (Draft) grounds it in **Promise Theory** [26]: a node is an autonomous agent that makes voluntary promises about its own state, and coordination emerges from compatible promises. The properties this buys are the ones a single-server platform already values: an agent converges toward its promised state idempotently and on its own schedule, while a partitioned agent keeps its promises locally, which is requirement R3 restated for a fleet. The declarative application spec is already the natural promise body, stating what the node undertakes to be running. The agent's cycle is to evaluate, compare and repair. This model is not new and it is not untested at scale; CFEngine [27] demonstrated it across very large estates.

ADR 017 stages the work so that each phase is useful alone:

1. **Single-node self-healing**: a reconciliation loop with health probes and restart policies, which pays off whether or not a second node ever exists (ADR 029, Draft).
...
There is a specific reason this rewrite is more tractable for this component than it would be elsewhere. The helper's input-validation contracts are already expressed as proof obligations checked independently of the implementation, exhaustively by a model checker as well as by property tests (§8.2). They exist because the same identifiers are validated in two places and a divergence between them would be a security defect. A reimplementation in another language must accept and reject exactly the same language, a property which can be *checked*. Having stated the contract separately from the code is what turns a privileged-component rewrite from an act of faith into a verifiable exercise.

### 9.3 Replacing the process runner

Applications are supervised by uWSGI in Emperor mode. It works. It is also the wrong long-term shape: a large multi-protocol application server used for a fraction of its capability, carrying configuration surface and a maintenance trajectory the platform does not control. ADR 023 (Draft) proposes replacing the runtime stack around it.
...
**A specification to test against.** The deeper problem is the one §7.6 identifies: unlike the build path, the command surface has no property that can be checked by machine, so each iteration was validated by use and revised when use disagreed. A stated model of resolution and precedence would change that: an object a test can be written against. We regard it, above the individual defects, as the substantive open item.

### 9.6 Composability with a control plane Hop3 does not own

The architecture assumes throughout that Hop3 owns its host: it provisions the databases, holds the secrets, writes the reverse-proxy configuration. That assumption governs exactly four plugin boundaries, each one the existing decomposition (§4.3) ought to be able to express:
...
The template system emits a Nix expression and a runtime description; the same machinery admits two extensions.
