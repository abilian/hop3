...
- **Encryption-at-rest of addon credentials.** Today addon-secrets are mode-0600 files on disk. Encryption needs a key that lives on the same box for daemon startup; without an HSM or operator-supplied per-deploy key, encryption-at-rest is theatre. Tracked in [ADR 011](./adrs/011-encryption.md) — out of scope for the 0.5 series.
- **Multi-node tenancy isolation.** Hop3 is single-host. Apps are isolated by Unix users + uWSGI vassals + nginx routing, not by VMs or containers. Tenants who don't trust each other should run on separate hosts.
- **Compliance certifications** (GDPR, ISO 27001, …). Hop3 ships primitives; operators certify deployments.
- **Strong-MFA, hardware-token enrolment, WebAuthn.** Tracked in [ADR 012](./adrs/012-mfa.md); not in 0.5.
...
**Boundary:** *App deployer → host.* The validator is the load-bearing control. If you are about to file "SQL injection in mysql.py", check that `validate_service_name` is still called both in `AddonCreateCmd.call` (`commands/services.py`) and in `__post_init__`.
...
**Why safe:** the password is generated by `secrets.token_hex(...)` and is hex-charset by construction. `_validate_mysql_password` is the canonical gate, called at the top of `_create_mysql_hop3_user`. The two-step contract (generation + re-validation at sink) is the load-bearing control; the SQL interpolation is only safe because the contract holds.
...
**Why safe:** the production override is the load-bearing control. Even an operator who deliberately exports `HOP3_UNSAFE=true HOP3_UNSAFE_ACK=yes-I-understand MODE=production` cannot disable auth in production. The order is enforced because `enforce_unsafe_mode_policy()` runs before any auth-guarded code path.
...
#### 3.4.5 CLI streaming RPC — bounded timeouts
...
- It is not a public security policy. There is no `report-vulnerability@…` channel committed to here, no SLA, no supported-versions matrix. Those belong in a root-level `SECURITY.md` if and when we publish them.
