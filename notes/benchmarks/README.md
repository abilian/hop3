...
- **Deploy tier** (build-and-install timing, baselines) — **mutates the target** and needs a *cold* store for honest "from scratch" numbers. Run only on a **throwaway, freshly-provisioned box**, never on a shared server.
...
- For provisioning: the `hcloud` CLI, `HETZNER_API_TOKEN`, `HETZNER_SSH_KEY_NAME`.
...
The deploy tier needs a throwaway box; these drive it. Measurement inside them goes through `hop3-bench`, so every stack shares one metric.

```bash
# 1. provision a throwaway box (prints its IP, waits for SSH)
IP=$(notes/benchmarks/scripts/provision.sh create hop3-bench-tmp)

# 2a. build-and-install timing (G1)
...
# 2b. or a control-plane baseline (use a FRESH box per stack — never co-resident)
notes/benchmarks/scripts/baseline-k3s.sh $IP
notes/benchmarks/scripts/baseline-compose.sh $IP

# 3. always tear the box down
notes/benchmarks/scripts/provision.sh delete hop3-bench-tmp
```

K3s and Docker must never share a box: both mutate global netfilter/iptables state, so a co-resident run inherits the other's contamination.
...
| `scripts/` | provisioning + deploy-timing + baseline orchestration |
