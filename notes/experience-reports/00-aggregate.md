...
**Nix-gen is 18 of 19** on that run. Easy!Appointments is its single failure: it builds its login form in JavaScript, so neither the HTTP check nor the browser harness can complete a sign-in, and its report says so instead of passing it on the strength of a bootstrap that reports success. That failure is stable and understood.
...
Native holds the corpus's only *20-application* all-green run; both figures now rest on complete recorded runs rather than on accumulated individual passes.
