...
**Status:** Final (0.7): the per-app reports are `report_status: final`; corrections continue against a published baseline.
...
**Nix-gen is 18 of 19** on that run. Easy!Appointments is its single failure: it builds its login form in JavaScript, so neither the HTTP check nor the browser harness can complete a sign-in. Its report records the failure. That failure is stable and understood.

**Hand-written Nix is 16 of 16** on a complete recorded run of its own (2026-08-01, browser harness). It read 15 of 16 earlier the same day, and the intervening hour is the more useful part of the record.

Bugsink had not installed. The reading that had gone unchallenged for weeks was that it was slow: three successive `start-timeout` increases across the three variants (120 → 180 → 240) had been spent on a failure whose headline was "did not start within Ns". It was not slow. Its gunicorn master bound the port eight seconds in and then no worker ever booted: no "Booting worker" line, no traceback, a silent hang for the remaining 232 seconds. The platform had diagnosed it correctly at every occurrence, in the line under the headline: *"the app's port is listening but it did not answer an HTTP request: the server bound its socket but no worker is serving."*

The cause was structural. **The hand-written recipe ran Bugsink as one process, and Bugsink is two.** Its two siblings each migrate a second database (`migrate --database=snappea`) and declare a second worker (`[run.workers] snappea = "bugsink-runsnappea"`), both carrying comments saying the application does not work without them; the hand-written recipe did neither, while generating its config from the same `--template docker` that assumes both. It could not have done otherwise: its setup lived inside the *web* start wrapper, and anything in that file runs only for gunicorn, so setup was unshareable with a second process by construction. Moving it to `before-run` and declaring the worker took the application from a 245-second hang to a 40-second pass.

Native holds the corpus's only *20-application* all-green run; both figures now rest on complete recorded runs.
...
Two moved the other way, deliberately. Paheko and Easy!Appointments now fetch the upstream *release* archive. Building from the git tag was dropped because the tag does not carry the whole application: vendored framework code in one case, built frontend assets and `vendor/` in the other. That is tier 3 by the table above. The alternative is a package that does not run.

## Open

- **A failure's headline outranked its diagnosis for weeks.** Bugsink's hand-written Nix recipe ran a two-process application as one process (fixed 2026-08-01, §What the numbers mean). Every occurrence printed both "failed to start within 240.0s" and, immediately below it, the correct diagnosis: "the server bound its socket but no worker is serving". Three timeout increases were spent on the first line. Nobody read the second. The platform's diagnostic was right and unread, which is a harder problem than a missing diagnostic; an argument for putting the specific finding *above* the generic timeout headline.
- **Docker is unclaimed.** Recipes exist and none has been measured at the sign-in bar; the reports no longer carry the variant. Both families that *have* been measured failed comprehensively on first contact, so this is a known unknown.
- **Easy!Appointments cannot be verified by either path.** Its bootstrap runs and reports success; the served page carries no form inputs for `check.py` to post, and the browser harness fills the JavaScript-built form and remains on it. It may need a check that queries the application directly.
