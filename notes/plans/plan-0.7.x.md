...
The presentation gap turned out to be a single defect. `catalog.toml` was **never read by hop3-server**: the loader asked for `[metadata].author/website/tags`, `[resources].memory`, `[port].web` and `[[provider]]`, none of which the recipes use. Measured on 2026-08-02, all 55 entries loaded with no tags, no memory, no services, no icon, and category `Other`; only title, description and version survived.
...
- [x] **SMTP credentials versus threat-model invariant 8. Decided 2026-08-11: the invariant was scoped, and the trip to the code found a defect.** Email was filed as *the* exception to "addon credentials are encrypted at rest"; it is one of four. `save_addon_secrets` writes plaintext for every addon, and Postgres and MySQL put the database password in it (`postgres.py:467`, `mysql.py:232`) just as email puts the SMTP one; that file, not the Fernet `AddonCredential` row, is the authoritative copy. Encrypting it would add no boundary: `/etc/hop3/secret-key` is 0640 `root:hop3` and apps run as `hop3`, so whoever can read the files can read the key. The real exposure is ADR 055's, already stated under invariant 9. Invariant 8 now says "in the control-plane database" and names the other store; security-model.md §3.4.7 is the new entry, and §3.4.5's "not as files on disk" and its "if you are about to file *credentials stored in plaintext*, don't" both had to go: the second would have waved off a true finding. **The defect:** the shared writer opened the file, wrote the password, *then* chmod'ed 0600, so it sat at 0644 (0666 under a loose umask) for every write and rotation, and since the open truncated first, a failed write destroyed a live credential the database still expected. Both reproduced against the old code. Now `mkstemp` + `os.replace`, 0600 from birth: the pattern `server/cli/setup.py` and the CLI credential store have used all along, at the one site that never caught up.
...
- [x] **Re-ran the audit over what the 2026-07-29 review's byte ceiling dropped** (2026-08-11, ~48k lines: `hop3-installer`, `hop3-testing`, `hop3-tui`, the email and Docker plugins, the Nix builder). No injection sink survived it: no `os.system`, no `eval`/`pickle`/`yaml.load`, no disabled TLS verification, no `extractall`, and the Nix generator's escaping is the best-reasoned code in the round. **Every finding was the same defect instead: something displayed that was never measured.** `hop3-tui`'s log viewer never called the API: it rendered eight hardcoded lines (`[ERROR] Failed to connect to redis` among them) and invented a new one every few seconds, and its download action would write those to a file an operator could attach to a bug report; `Hop3Client.get_app_logs` existed the whole time and is now called. Its system screen reported 42%/63%/81% as constants on a live-looking timer, `hop3.dev v0.5.0` with 14 days uptime, and a services panel that discarded the data passed to it and rendered four services RUNNING unconditionally; those now say "not reported by the server" until the client parses the response. And `hop3-testing`'s `add_host_key` built a shell command string with `2>/dev/null`, so a failing host-key scan returned `False` with the reason discarded. Fifteen tests. Deferred with reasons: unpinned Nix-installer and `get-pip.py` downloads (verified HTTPS, no checksum). Written up in [`../security/backlog-2026-08-01.md`](../security/backlog-2026-08-01.md) §5.
...
Harvested: the overlay loader (ported as a standalone change, with its log-and-continue on a malformed overlay changed to a refusal), the `HOP3_SECRET_KEY` length check, the reconciler timeout wiring, and the Forgejo licence. Superseded and dropped: stateful sessions, `serve --workers` (which would now fight the single-worker rate-limiter invariant), two migrations, and the magic-link URL change.
