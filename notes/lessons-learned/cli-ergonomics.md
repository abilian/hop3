...
- `app env` → folded into `env show --sources`; `AppEnvCmd` kept `hidden = True`.
- `app build-logs` → folded into `app logs --build`; kept `hidden = True`.
- `auth register` → an admin-gated subset of `user add`; deleted the class, kept `("auth", "register")` as an alias of `user add`.

The catalog the client fetches filters `hidden`; the dispatch table keeps it. Test both invariants: hidden commands don't appear in the catalog, and still resolve when invoked.
