...
Everything below the "Topic deep dives" list is legacy, predating the thematic-file convention: the numbered "Quick Reference" table and the full inline lessons. The topic files are canonical; several inline lessons are already superseded in those files.
...
- [`web-auth-and-csrf.md`](./web-auth-and-csrf.md) - cookie-based CSRF/session auth pitfalls (Litestar): why rotating a password wedged login permanently (`CSRF token verification failed`), self-healing on failure, never showing raw JSON to a browser; then the August 2026 round - why a rate limit belongs to the operation (and must share one *instance*), making all three login failures indistinguishable including in time, the `Secure`-cookie-over-plain-HTTP silent loop, and why "every mutation is a POST" needs a test.

---

## Quick Reference (numbered lesson index)

A numeric-ordered lookup into the theme-grouped inline lessons below (which are otherwise ordered by topic). Some entries are now superseded by a topic file and duplicated here only for history: 2, 6, 7 → `multi-distribution-support.md`; 18 → `async-thread-boundaries.md`. The rest have no topic-file home yet. Each should migrate when a fitting thematic file is created.
...
**Best practices**:
- Review inherited code critically; test with validation modes enabled

### 10. Avoid Generic Error Detection Patterns

**Lesson**: When detecting errors in logs or output, use specific patterns. Generic patterns like "error" will match normal log output.
...
**Best practices**:
- Use `self-signed` certificates by default; switch to `certbot` for production
...
**Lesson**: Package names and repository availability vary widely across distributions.
...
**Lesson**: When a package behaves differently across distributions (different versions, build options, or plugin architectures), install it via pip.
...
- Version is controlled by the project
...
**Lesson**: When ORM models need to call business logic that imports the same ORM, use lazy imports inside methods.
...
### 13. Verify Deployment Success by Checking the Port
...
Docker-based apps using Docker Compose couldn't connect to the host PostgreSQL because pg_hba.conf only allowed 172.17.0.0/16; Compose used 192.168.x.x addresses.
...
**Lesson**: The server/installer should provide base language runtimes. Tutorials install their own dependencies as part of setup.
