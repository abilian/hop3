...
How a background thread talking to an `asyncio` primitive made *every* deployment appear to take ~30s, why it was subtle, how it was actually diagnosed (after a wrong first guess), and the rule that prevents the whole class.
...
See `packages/hop3-server/src/hop3/server/streaming.py` and the regression test `test_cross_thread_notify_is_scheduled_on_the_loop` in `tests/a_unit/test_streaming.py`.
...
2. A slow-SQL listener (`before/after_cursor_execute`) logged **nothing** ≥2s → no statement, and crucially `commit()` is *not* a cursor execute, so a slow commit would have been invisible — but timing the commit showed 0.0s. (Killed the DB-lock theory.)
