...
When a test runner displays "Tests to run: 10" but actually runs 78, the bug is that filtering was applied during planning but not during execution; the two code paths diverged.
...
Infer the runner type from the path:
...
Diagnostics must be collected BEFORE the app is destroyed. The test runner cleans up apps after each test, and any diagnostics collection that runs after cleanup will find empty log directories.
...
When upstream has no prebuilt binary, document the constraint in a per-app `DEFERRED.md` under `apps/bad/`.
...
**Fix in test infrastructure:** for apps where authentication is built in, point health checks at a public endpoint that exists regardless of auth state (Stirling-PDF: `/login`; Vaultwarden: `/alive`; Forgejo: `/api/healthz`; GoToSocial: `/api/v1/instance`). Use the endpoint the upstream project documents as a liveness probe.
...
**Watch for a backend that recreates state every deploy.** `DockerDeployBackend.setup()` unconditionally removed and recreated the container, so a second `--docker` deploy was *always* a fresh install - the in-place update path was unreachable, and an "upgrade" test on docker silently tested fresh-install-twice. The fix was to honour `--clean` (reuse a running container when it's absent). If a "passing" e2e can't actually reach the code path it names, the test is a no-op. (Related: the deployer's `systemctl restart` was a silent no-op under supervisor, faking success - service ops must be process-manager-aware.)

**Some releases are not viable baselines.** `hop3-rootd 0.6.0` imports `__version__` that its `__init__` never defines - in the published wheel *and* the tag source - so it can't start. An upgrade chain simply excludes such a version, and that exclusion is itself a data point about the release's quality.
