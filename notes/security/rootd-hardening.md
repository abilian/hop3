...
matching the proven-working container/supervisor model.

**Why this is still open (not closed as "won't do"):** one of the two items
below is a live privilege-escalation path. The other is defence in depth.
They are recorded together because the same relocation unblocks both.
...
| `CapabilityBoundingSet=CAP_NET_ADMIN` (drops `CAP_DAC_OVERRIDE`) | `nginx -t` → `[emerg] … failed (13: Permission denied)` on `/var/log/nginx/error.log` | rootd runs `nginx -t` as root but **without** `CAP_DAC_OVERRIDE`, so it cannot open nginx's `www-data`-owned error log. This is a DAC failure; the filesystem is read-write. |
...
- **Test-harness blind spot (since closed).** The system test's HTTP check hit
  the app's *direct port*, bypassing the nginx vhost, so uWSGI apps passed while
  their vhosts never applied. Only the static app, which has no direct port, surfaced
...
only the namespace *view*; it does **not** protect the host source or fix the
escalation.
...
change unblocks both items, which is why it is first.
...
that let this ship broken. It does not yet drive a privileged
...
the minimal unit is a deliberate decision, and the test makes re-adding
